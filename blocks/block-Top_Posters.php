...
if (intval($nxp_toppost_post) == 1)
{
	...
	$nxp_toppost_order_by  = "nuke_user_posts DESC";
}
...
$Q02 = "SELECT user_id, username, user_avatar, user_from, user_from_flag, nuke_user_posts, nuke_user_rank FROM " . $prefix . "_users WHERE `user_id` != ".ANONYMOUS." ORDER BY " . $nxp_toppost_order_by . " LIMIT 0," . $nxp_toppost_show_numb;
...
while($nxp_toppost_user_info = $db->sql_fetchrow($R02))
{
	...
	// Sort Out Rank Title ---------------------------------------------------------|
	$cur_nuke_user_rank  = intval($nxp_toppost_user_info['nuke_user_rank']);
	$cur_nuke_user_posts = intval($nxp_toppost_user_info['nuke_user_posts']);

	if ($cur_nuke_user_rank != 0)
	{
		$i = 0;
		while($nxp_toppost_rank_spec[$i][0] != "")
		{
			if ($nxp_toppost_rank_spec[$i][0] == $cur_nuke_user_rank)
			{
	...
	elseif ($cur_nuke_user_rank == 0)
	{
		...
		while($nxp_toppost_rank_norm[$i][0] != "")
		{
			...
			if ($cur_nuke_user_posts >= $nxp_toppost_rank_norm[$i][2] && $cur_nuke_user_posts < $nxp_toppost_rank_norm[$j][2])
			{
	...
	$content .= "						" . number_format(intval($nxp_toppost_user_info['nuke_user_posts']), 0) . "</td>\n";
	$content .= "				</tr>\n";
	$content .= "			</table>";
	$content .= "		</td>\n";
	$content .= "	</tr>\n";
