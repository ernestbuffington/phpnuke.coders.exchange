...
// Check if Description exist
    ...
    $filesize = preg_replace('#\.#m',"",$filesize);
    $filesize = preg_replace('#,#m',"",$filesize);
    $cat[0] = intval($cat[0]);
    $cat[1] = intval($cat[1]);
    ...
    if ($num_new == 0) {
    $db->sql_query("INSERT INTO ".$prefix."_downloads_newdownload VALUES (NULL, 
	                                                                 '$cat[0]', 
																	 '$cat[1]', 
													  '".addslashes($title)."', 
													    '".addslashes($url)."', 
    ...
    $result=$db->sql_query("SELECT lid, 
				 ...
				 downloadratingsummary, 
						 ...
						 totalcomments, 
						      filesize, 
							   version, 
							  homepage FROM ".$prefix."_downloads_downloads WHERE cid='$cid' ORDER BY $orderby limit $min, $perpage ");
...
function convertorderbytrans($orderby) {
   ...
   if ($orderby != "hits ASC" 
       AND $orderby != "hits DESC" 
	   AND $orderby != "title ASC" 
	   AND $orderby != "title DESC" 
	   AND $orderby != "date ASC" 
	   AND $orderby != "date DESC" 
	   AND $orderby != "downloadratingsummary ASC" 
	   AND $orderby != "downloadratingsummary DESC") {
...
function search($query, $min, $orderby, $show) {
    ...
    if (!empty($query)) {
	    ...
	    while(list($cid, $stitle) = $db->sql_fetchrow($result2)) {
    	    ...
    	    $title3 = preg_replace('#' . preg_quote($query, '#') . '#m', "<b>$query</b>", $title3);
	    ...
	    while(list($lid, $cid, $title, $url, $description, $time, $hits, $downloadratingsummary, $totalvotes, $totalcomments, $filesize, $version, $homepage) = $db->sql_fetchrow($result)) {
            ...
            $description = filter($description);	    
			$transfertitle = str_replace (" ", "_", $title);
			$title = preg_replace('#' . preg_quote($query1, '#') . '#m', "<b>$query1</b>", $title);
		...
		$description = preg_replace('#' . preg_quote($the_query, '#') . '#m', "<b>$the_query</b>", $description);
...
function viewdownloaddetails($lid) {
	...
	if (empty($email)) {
	    ...
	    $email = preg_replace('#@#m'," <i>at</i> ",$email);
	    $email = preg_replace('#\.#m'," <i>dot</i> ",$email);
