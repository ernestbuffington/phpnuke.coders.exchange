...
//
include($phpbb_root_path.'language/lang_' . $userdata['nuke_user_lang'] . '/lang_user_delete.'.$phpEx);
...
if (($mode == 3) || ($mode == 4))
{
	...
	WHERE poster_id = $user_id";
	  ...
	  if ( ( $nuke_user_posts_in_other_topics = $db->sql_numrows($result)) > '0' )
	  {
	        ...
	        if (!empty($topics_with_posts_of_user_only_sql))
	        {
	          $sql = "SELECT topic_id, topic_title, forum_id
	            ...
	            $topics_with_nuke_user_posts_num = $db->sql_numrows($result);
	            $topics_with_nuke_user_posts = $db->sql_fetchrowset($result);
	          ...
	          $sql = "SELECT post_id, topic_id
	            ...
	            $posts_in_topics_with_only_nuke_user_posts_num = $db->sql_numrows($result);
	            $posts_in_topics_with_only_nuke_user_posts = $db->sql_fetchrowset($result);

                $deleted_posts_in_u_topics2_id = '';
                $posts2_del_sql = '';
                foreach($posts_in_topics_with_only_nuke_user_posts as $val)
	        	{
                    //Resync forum
                    forum_postscount_decrease_by_post_id($val['post_id']);
                ...
                $template->assign_block_vars('topics_where_only_this_nuke_user_posts', array(
...
//##################################################################
    ...
    if ( ( $nuke_user_posts_in_other_topics = $db->sql_numrows($result)) > '0' )
    {
	        ...
	        $template->assign_block_vars('all_other_nuke_user_posts_in_other_topics', array(
