...
// forums
 $sql = "SELECT ug.user_id, f.forum_id, f.forum_name
           ...
           LEFT JOIN  " . AUTH_ACCESS_TABLE . " aa2 ON aa2.nuke_group_id = ug2.nuke_group_id AND aa2.auth_view = " . TRUE . ",
...
while($staff = $db->sql_fetchrow($results))
{
        ...
        {
                switch( $staff['nuke_user_avatar_type'] )
                {
        ...
        $result = $db->sql_query('SELECT group_name FROM '. AUC_TABLE .' WHERE nuke_group_id='.$lvl);
        ...
        //Groups
        $result = $db->sql_query("SELECT group_name FROM " . GROUPS_TABLE . " g LEFT JOIN " . USER_GROUP_TABLE . " ug on ug.nuke_group_id=g.nuke_group_id WHERE ug.user_id='".$staff['user_id']."' and g.group_description != 'Personal User'");
        ...
        $regdate = $staff['nuke_user_regdate'];
        $nukedate = strtotime($regdate);
        $memberdays = max(1, round( ( time() - $nukedate ) / 86400 ));
        $posts_per_day = $staff['nuke_user_posts'] / $memberdays;
        if ( $staff['nuke_user_posts'] != 0 )
        {
                $total_posts = get_db_stat('postcount');
                $percentage = ( $total_posts ) ? min(100, ($staff['nuke_user_posts'] / $total_posts) * 100) : 0;
        }
        else
        {
                $percentage = 0;
        ...
        $rank = '';
        $rank_image = '';
        if ( $staff['nuke_user_rank'] )
        {
                for($j = 0; $j < count($ranksrow); $j++)
                {
                        if ( $staff['nuke_user_rank'] == $ranksrow[$j]['rank_id'] && $ranksrow[$j]['rank_special'] )
                        {
        ...
        else
        {
                for($j = 0; $j < count($ranksrow); $j++)
                {
                        if ( $staff['nuke_user_posts'] >= $ranksrow[$j]['rank_min'] && !$ranksrow[$j]['rank_special'] )
                        {
                                $rank = $ranksrow[$j]['rank_title'];
                                $rank_image = ( $ranksrow[$j]['rank_image'] ) ? '<img src="modules/Forums/' . $ranksrow[$j]['rank_image'] . '" alt="' . $rank . '" title="' . $rank . '" border="0" /><br />' : '';
                        }
 ...
 ******************************************************/
                'LEVEL' => $level,
                'FORUMS' => $forums,
                'JOINED' => $staff['nuke_user_regdate'],
                'PERIOD' => sprintf($lang['Period'], $memberdays),
                'POSTS' => $staff['nuke_user_posts'],
