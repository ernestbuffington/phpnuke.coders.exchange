...
$nuke_group_id = $_POST[POST_GROUPS_URL] = $_POST[POST_GROUPS_URL] ?? '';

if ( !empty($nuke_group_id) )
{
    if( $nuke_group_id != 'users' && $nuke_group_id != 'admins' && $nuke_group_id != 'moderators')
    {
        $sql = "SELECT DISTINCT g.group_name
            FROM ".GROUPS_TABLE . " g, ".USER_GROUP_TABLE . " ug
            WHERE g.group_single_user <> 1 AND g.nuke_group_id='".$nuke_group_id."'
            AND (
        ...
        $sql = "SELECT distinct u.user_id, u.nuke_user_lang, u.user_email, u.username, u.nuke_user_notify_pm,u.user_active,u.user_allow_mass_pm
            FROM " . USERS_TABLE . " u, " . USER_GROUP_TABLE . " ug
            WHERE u.user_allow_mass_pm > 1 AND ug.nuke_group_id = $nuke_group_id
                AND ug.user_pending <> " . TRUE . "
                AND u.user_id <> " . ANONYMOUS . "
                AND u.user_id = ug.user_id
                ORDER BY u.nuke_user_lang";
    }
    elseif ($nuke_group_id == 'users')
    {
        if( $userdata['user_level']!=ADMIN ) message_die(GENERAL_ERROR, $lang['Not_Authorised']);
        $sql = "SELECT distinct user_id, nuke_user_lang, user_email, username, nuke_user_notify_pm,user_active,user_allow_mass_pm
            FROM " . USERS_TABLE." WHERE user_allow_mass_pm > 1
                    AND user_id <> " . ANONYMOUS." ORDER BY nuke_user_lang";
        $group_name=$lang['All_users'];
    }
    elseif ($nuke_group_id == 'admins')
    {
        if( $userdata['user_level']!=ADMIN ) message_die(GENERAL_ERROR, $lang['Not_Authorised']);
        $sql = "SELECT distinct user_id, nuke_user_lang, user_email, username, nuke_user_notify_pm,user_active,user_allow_mass_pm
            FROM " . USERS_TABLE." WHERE user_allow_mass_pm > 1
                    AND user_level = '2' ORDER BY nuke_user_lang";
        $group_name=$lang['All_admins'];
    }
    elseif ($nuke_group_id == 'moderators')
    {
        if( $userdata['user_level']!=ADMIN ) message_die(GENERAL_ERROR, $lang['Not_Authorised']);
        $sql = "SELECT distinct user_id, nuke_user_lang, user_email, username, nuke_user_notify_pm,user_active,user_allow_mass_pm
            FROM " . USERS_TABLE." WHERE user_allow_mass_pm > 1
                    AND user_level = '3' ORDER BY nuke_user_lang";
 ...
 ******************************************************/
    ...
    $sql = "SELECT DISTINCT g.nuke_group_id, g.group_name
    FROM ".GROUPS_TABLE . " g, ".USER_GROUP_TABLE . " ug
    ...
    {
        $select_list .= '<option value = "' . $group_list[$i]['nuke_group_id'].'"'. (($group_list[$i]['nuke_group_id']==$groupname) ? ' SELECTED ' : '').'>'.$group_list[$i]['group_name'] .'</option>';
    }
    $select_list .= "</select>";
