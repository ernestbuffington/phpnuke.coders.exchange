...
//
function user_is_within_group($user_id, $nuke_group_id)
{
    global $db;

    if ((empty($user_id)) || (empty($nuke_group_id)))
    {
        return (FALSE);
    }
    
    $sql = "SELECT u.nuke_group_id FROM " . USER_GROUP_TABLE . " u, " . GROUPS_TABLE . " g 
    WHERE (g.group_single_user = 0) AND (u.nuke_group_id = g.nuke_group_id) AND (u.user_id = " . $user_id . ") AND (g.nuke_group_id = " . $nuke_group_id . ")
    LIMIT 1";
...
function get_modules($activated = true, $module_id = -1)
{
    ...
    {
        $authed_groups[$g_rows[$i]['module_id']][] = $g_rows[$i]['nuke_group_id'];
    }
