...
switch( $mode )
{
    case 'delete':
        ...
        else
        {
            ...
            while( $i < count($user_ids) )
            {
                ...
                $sql = "SELECT u.username, g.nuke_group_id
    ...
    case 'group':
        ...
        if ( !$confirm )
        {
            ...
            $sql = "SELECT nuke_group_id, group_name FROM " . GROUPS_TABLE . "
    ...
    default:
        ...
        $template->assign_vars(array(
			...
			'SELECTED_JOINED' => ($sort=="nuke_user_regdate") ? " selected" : "",
			'SELECTED_ACTIVTY' => ($sort=="user_session_time") ? " selected" : "",
			'SELECTED_USER_LEVEL' => ($sort=="user_level") ? " selected" : "",
			'SELECTED_POSTS' => ($sort=="nuke_user_posts") ? " selected" : "",
			'SELECTED_RANK' => ($sort=="nuke_user_rank") ? " selected" : "",
        ...
        while ( $row = $db->sql_fetchrow($result) )
        {
            ...
            //
            $avatar_img = '';
            if ( $row['nuke_user_avatar_type'] && $row['user_allowavatar'] )
            {
 ...
 ******************************************************/
            ...
            $poster_rank = '';
            $rank_image = '';
            if ( $row['nuke_user_rank'] )
            {
                for($ji = 0; $ji < count($ranksrow); $ji++)
                {
                    if ( $row['nuke_user_rank'] == $ranksrow[$ji]['rank_id'] && $ranksrow[$ji]['rank_special'] )
                    {
            ...
            else
            {
                for($ji = 0; $ji < count($ranksrow); $ji++)
                {
                    if ( $row['nuke_user_posts'] >= $ranksrow[$ji]['rank_min'] && !$ranksrow[$ji]['rank_special'] )
                    {
                        $poster_rank = $ranksrow[$ji]['rank_title'];
                        $rank_image = ( $ranksrow[$ji]['rank_image'] ) ? '<img src="../' . $ranksrow[$ji]['rank_image'] . '" alt="' . $poster_rank . '" title="' . $poster_rank . '" border="0" /><br />' : '';
                    }
 ...
 ******************************************************/
                    ...
                    'U_GROUP' => ("../../../modules.php?name=Groups&amp;g=$group_row[nuke_group_id]"))
                );
                $g++;
            }
