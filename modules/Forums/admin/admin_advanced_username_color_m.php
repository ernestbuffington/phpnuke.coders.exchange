 ...
 ******************************************************/
    ...
    $q = "SELECT nuke_group_id, group_name
          FROM ". GROUPS_TABLE ."
          WHERE nuke_group_id > 0 AND group_description <> 'Personal User'
          ORDER BY nuke_group_id ASC";
    $r = $db->sql_query($q);
    $groups = $db->sql_fetchrowset($r);

    echo "                <select name='nuke_group_id'>";
    echo '                    <option selected value="" class="post">----------</option>';
        for ($x = 0; $x < count($groups); $x++)
            echo '<option value="'. $groups[$x]['nuke_group_id'] .'" class="post">'. $groups[$x]['group_name'] .'</option>';
    echo "                </select>";
    echo "            </span>";
    echo "        </td>";
    echo '    </tr>';
 ...
 ******************************************************/
    ...
    $groups_info = $db->sql_fetchrowset($r);
        for ($a = 0; $a < count($groups_info); $a++)
            {
            if (preg_match('/--'. $groups_info[$a]['nuke_group_id'] .'--/i', $new_id))
                {
 ...
 ******************************************************/
    if ($mode == "add_group")
        {
    $who    = intval($HTTP_POST_VARS['nuke_group_id']);
    $group     = intval($HTTP_POST_VARS['group']);
    $color     = $HTTP_POST_VARS['color'];

        $q_group = "SELECT user_id
              FROM ". USER_GROUP_TABLE ."
              WHERE nuke_group_id = '". $who ."'";
