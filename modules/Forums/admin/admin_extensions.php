...
if ($submit && $mode == 'extensions')
{
    ...
    for ($i = 0; $i < sizeof($extension_change_list); $i++)
    {
        ...
        $extensions['_' . $extension_change_list[$i]]['nuke_group_id'] = intval($group_select_list[$i]);
    }
    ...
    if ($num_rows > 0)
    {
        ...
        {
            if ($extension_row[$i]['comment'] != $extensions['_' . $extension_row[$i]['ext_id']]['comment'] || intval($extension_row[$i]['nuke_group_id']) != intval($extensions['_' . $extension_row[$i]['ext_id']]['nuke_group_id']))
            {
                $sql_ary = array(
                    'comment'        => (string) $extensions['_' . $extension_row[$i]['ext_id']]['comment'],
                    'nuke_group_id'        => (int) $extensions['_' . $extension_row[$i]['ext_id']]['nuke_group_id']
                );
    ...
    if ($extension != '' && $add)
    {
        ...
        if (!isset($error))
        {
            ...
            {
                $sql_ary = array(
                    'nuke_group_id'        => (int) $extension_group,
        ...
        for ($i = 0; $i < $num_extension_row; $i++)
        {
            ...
            else
            {
                $template->assign_block_vars('extension_row', array(
                    ...
                    'S_GROUP_SELECT'    => group_select('group_select[]', $extension_row[$i]['nuke_group_id']))
                );
            }
        }
    }
...
if ($submit && $mode == 'groups')
{
    ...
    for ($i = 0; $i < sizeof($group_change_list); $i++)
    {
        ...
        $sql = 'UPDATE ' . EXTENSION_GROUPS_TABLE . ' SET ' . attach_mod_sql_build_array('UPDATE', $sql_ary) . '
            WHERE nuke_group_id = ' . (int) $group_change_list[$i];
    ...
    $nuke_group_id_list = get_var('nuke_group_id_list', array(0));

    $nuke_group_id_sql = implode(', ', $nuke_group_id_list);

    if ($nuke_group_id_sql != '')
    {
        $sql = 'DELETE
            FROM ' . EXTENSION_GROUPS_TABLE . '
            WHERE nuke_group_id IN (' . $nuke_group_id_sql . ')';
...
if ($mode == 'groups')
{
    ...
    for ($i = 0; $i < $num_extension_group; $i++)
    {
        ...
        $template->assign_block_vars('grouprow', array(
            'GROUP_ID'            => $extension_group[$i]['nuke_group_id'],
            'EXTENSION_GROUP'    => $extension_group[$i]['group_name'],
            'UPLOAD_ICON'        => $extension_group[$i]['upload_icon'],

            'S_ALLOW_SELECTED'    => $s_allowed,
            'S_SELECT_CAT'        => category_select('category_list[]', $extension_group[$i]['nuke_group_id']),
            'S_DOWNLOAD_MODE'    => download_select('download_mode_list[]', $extension_group[$i]['nuke_group_id']),
            'S_FILESIZE'        => size_select('size_select_list[]', $size_format),

            'MAX_FILESIZE'        => $extension_group[$i]['max_filesize'],
            'CAT_BOX'            => ($viewgroup == $extension_group[$i]['nuke_group_id']) ? $lang['Decollapse'] : $lang['Collapse'],
            'U_VIEWGROUP'        => ($viewgroup == $extension_group[$i]['nuke_group_id']) ? append_sid("admin_extensions.$phpEx?mode=groups") : append_sid("admin_extensions.$phpEx?mode=groups&amp;" . POST_GROUPS_URL . "=" . $extension_group[$i]['nuke_group_id']),
            'U_FORUM_PERMISSIONS'    => append_sid("admin_extensions.$phpEx?mode=$mode&amp;e_mode=perm&amp;e_group=" . $extension_group[$i]['nuke_group_id']))
        );

        if ($viewgroup && $viewgroup == $extension_group[$i]['nuke_group_id'])
        {
            $sql = 'SELECT comment, extension
                FROM ' . EXTENSIONS_TABLE . '
                WHERE nuke_group_id = ' . (int) $viewgroup;
...
if (isset($add_forum) && $e_mode == 'perm' && $group)
{
    ...
    if ($add_all_forums)
    {
        $sql = 'UPDATE ' . EXTENSION_GROUPS_TABLE . " SET forum_permissions = '' WHERE nuke_group_id = " . (int) $group;
        if (!($result = $db->sql_query($sql)))
        {
    ...
    if (!$add_all_forums)
    {
        $sql = 'SELECT forum_permissions
            FROM ' . EXTENSION_GROUPS_TABLE . '
            WHERE nuke_group_id = ' . intval($group) . '
            LIMIT 1';
        ...
        $sql = 'UPDATE ' . EXTENSION_GROUPS_TABLE . " SET forum_permissions = '" . attach_mod_sql_escape($auth_bitstream) . "' WHERE nuke_group_id = " . (int) $group;
