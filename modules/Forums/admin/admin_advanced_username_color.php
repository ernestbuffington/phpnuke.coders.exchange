...
include($phpbb_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_auc.' . $phpEx);
    ...
    while ($rows = $db->sql_fetchrow($r))
        {
        if ($rows['nuke_group_id'])
            {
        $q1 = "UPDATE ". COLORS ."
               SET group_weight = '". $rows['nuke_group_id'] ."'
               WHERE nuke_group_id = '". $rows['nuke_group_id'] ."'";
        $db->sql_query($q1);
            }
        }
...
if($mode == "main" || !$mode)
    ...
    while($row     = $db -> sql_fetchrow($r))
        {    
        if ($w != 1)
            $links = '&nbsp;&nbsp;<a href="admin_advanced_username_color.'. $phpEx .'?mode=move_up&amp;id='. $row['nuke_group_id'] .'&amp;sid='. $userdata['session_id'] .'">Up</a>';
        else
            $links = '&nbsp;&nbsp;<a href="admin_advanced_username_color.'. $phpEx .'?mode=move_down&amp;id='. $row['nuke_group_id'] .'&amp;sid='. $userdata['session_id'] .'">Down</a>';
    ...
    $row     = $db->sql_fetchrowset($r);
        ...
        for ($a = 0; $a < count($row); $a++)
            ...
            if (!$remove)
                ...
                if ($new_id)
                    {
                $q = "SELECT group_color, nuke_group_id
                      FROM ". COLORS ."";
                $r = $db->sql_query($q);
                $groups_info = $db->sql_fetchrowset($r);
                    ...
                    for ($b = 0; $b < count($groups_info); $b++)
                        {
                        if (preg_match('/--'. $groups_info[$b]['nuke_group_id'] .'--/i', $new_id))
                            {
 ...
 ******************************************************/
    ...
    echo "            <input type='hidden' name='id' value='". $row['nuke_group_id'] ."'>";                    
