...
else
{
  ...
  while( $row = $db->sql_fetchrow($result) )
  {
    $is_selected = (in_array($row['nuke_group_id'],$ad_no_groups_current)) ? 'selected="selected"' : '';
    $ad_no_groups .= '<option value="' . $row['nuke_group_id'] . '" ' . $is_selected . '>' . $row['group_name'] . '</option>';
  }
