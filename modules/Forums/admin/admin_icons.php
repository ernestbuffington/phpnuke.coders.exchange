	...
	if ($mode == 'del')
	{
     ...
     $tmpCount = count($tmp);
					for ($j=0; ( ($j < $icon_per_row) && ($offset < $tmpCount) ); $j++)
					{
						...
						$template->assign_block_vars('replace.row.cell', ['ICON_ID'	=> $tmp[$offset]['ind'], 'ICON_CHECKED'	
						                                                            => ($tmp[$offset]['ind'] == 0) ? ' checked="checked"' : '', 'ICON_IMG'		
																					=> get_icon_title($tmp[$offset]['ind'], 2, -1, true)]
						);
						$offset++;
					}
				}
