...
if (isset($_POST['add_group']) && $mode == 'mod_edit')
{
    ...
    if ( (!$nuke_group_id) || (empty($nuke_group_id)) )
    {
    ...
    $sql = "INSERT INTO " . MODULE_GROUP_AUTH_TABLE . " (module_id, nuke_group_id) VALUES (" . $module_id . ", " . $nuke_group_id . ")";
...
{
    $nuke_group_id = (int) $_POST['added_group'];
    ...
    $sql = "DELETE FROM " . MODULE_GROUP_AUTH_TABLE . " WHERE module_id = " . $module_id . " AND nuke_group_id = " . $nuke_group_id;
...
if ($mode == 'mod_edit') {
    ...
    {
        $nuke_group_ids[] = $group_name[$i]['nuke_group_id'];
    }
    $sql = "SELECT g.nuke_group_id, g.group_name
    FROM " . GROUPS_TABLE . " g, " . MODULE_GROUP_AUTH_TABLE . " m
    WHERE m.nuke_group_id = g.nuke_group_id AND m.module_id = " . (int) $mod_info['module_id'] . "
    ORDER BY group_name";
    if ( !($result = $db->sql_query($sql)) )
    {
        message_die(GENERAL_ERROR, "Couldn't query Groups Table", "", __LINE__, __FILE__, $sql);
    ...
    if (($num_groups > 0) && ($num_rows > 0))
    {
        for ($i = 0; $i < $num_rows; $i++)
        {
            if (!in_array($rows[$i]['nuke_group_id'], $nuke_group_ids))
            {
                $sql = "DELETE FROM " . MODULE_GROUP_AUTH_TABLE . " WHERE module_id = " . $module_id . " AND nuke_group_id = " . $rows[$i]['nuke_group_id'];
    ...
    for ($i = 0; $i < $num_rows; $i++)
    {
        $added_groups['nuke_group_id'][] = $rows[$i]['nuke_group_id'];
    ...
    if ( $num_groups > 0 )
    {
        ...
        foreach ($group_name as $i => $singleGroup_name) {
			...
			if (count($added_groups['nuke_group_id']) == 0) {
                $add = TRUE;
            } 
			elseif (!in_array($singleGroup_name['nuke_group_id'], $added_groups['nuke_group_id'])) 
			{
                $add = TRUE;
            }
            endif;
			...
			if ($add)
            {
                ...
                $group_select .= '<option value="' . $singleGroup_name['nuke_group_id'] . '"' . $selected . '>' . $singleGroup_name['group_name'] . '</option>';
                $act_group++;
            }
        }
        $group_select .= '</select>';
	...
	if (is_countable($added_groups['nuke_group_id']) && count($added_groups['nuke_group_id']) > 0) :
	if (count($added_groups['nuke_group_id']) == 0)
    {
    ...
    else
    {
        ...
        $itemsCount = count($added_groups['nuke_group_id']);

        for($i = 0; $i < $itemsCount; $i++)
        {
            $selected = ($i == 0) ? ' selected="selected"' : '';
            $group_added_select .= '<option value="' . $added_groups['nuke_group_id'][$i] . '"' . $selected . '>' . $added_groups['group_name'][$i] . '</option>';
        }
