 ...
 ******************************************************/
                $initialgroup = $board_config['initial_nuke_group_id'];
/*****[END]********************************************
 [ Base:     Evolution Functions               v1.5.0 ]
 ******************************************************/
                ...
                if (intval($initialgroup) == $nuke_group_id) {
                   $yesno = 1;
                } else {
                   $yesno = 0;
                }
 ...
 ******************************************************/
 ...
 [ Mod:     Group Colors                       v1.0.0 ]
 ******************************************************/
        $group_color = auc_colors_select($group_info['group_color'], "group_color", "nuke_group_id");
 ...
 ******************************************************/
                $sql = "SELECT config_value
                        FROM " . CONFIG_TABLE . "
                        WHERE config_name='initial_nuke_group_id'";
                $result = $db->sql_query($sql);
                $row = $db->sql_fetchrow($result);
                $initialgroup = $row[0];

                if ($initialgroup == $nuke_group_id) {
 ...
 ******************************************************/
                        if(isset($_POST['initial_group'])) {
                            if($_POST['initial_group'] == 1) {
                                $sql = $sql = "UPDATE
                                " . CONFIG_TABLE . "
                                 SET config_value = '".$nuke_group_id."'
                                WHERE config_name ='initial_nuke_group_id'";
                                $result = $db->sql_query($sql);
 ...
 ******************************************************/
        			...
        			{
        			    remove_group_attributes('', $nuke_group_id);
        				//removing old users
        				$sql = "DELETE FROM " . USER_GROUP_TABLE . "
        					WHERE nuke_group_id=$nuke_group_id 
        			...
        			if ( $group_count_update)
        			{
        				...
        				$sql = "SELECT u.user_id FROM " . USERS_TABLE . " u
        					LEFT JOIN " . USER_GROUP_TABLE ." ug ON u.user_id=ug.user_id AND ug.nuke_group_id='$nuke_group_id'
        					WHERE u.nuke_user_posts>='$group_count' AND u.nuke_user_posts<'$group_count_max'
        					AND ug.nuke_group_id is NULL
        				...
        				{
        					$sql = "INSERT INTO " . USER_GROUP_TABLE . " (nuke_group_id, user_id, user_pending) 
        						VALUES ($nuke_group_id, " . $new_members['user_id'] . ", 0)";
        					if ( !($result2 = $db->sql_query($sql)) )
        					{
        						message_die(GENERAL_ERROR, 'Error inserting user group, group count', '', __LINE__, __FILE__, $sql);
        					}
        					add_group_attributes($new_members['user_id'], $nuke_group_id);
        					$group_count_added++;
        				}
        			}
/*****[END]********************************************
                		...
                		if ($group_count_delete)
                			{
                				//removing old users
                				$sql = "DELETE FROM " . USER_GROUP_TABLE . "
                					WHERE nuke_group_id=$new_nuke_group_id 
                					AND user_id NOT IN ('$group_moderator','".ANONYMOUS."')";
                				if ( !$db->sql_query($sql) )
                				{
                					message_die(GENERAL_ERROR, 'Could not remove users, group count', '', __LINE__, __FILE__, $sql);
                			...
                			if ( $group_count_update)
                			{
                				//finding new users
                				$sql = "SELECT u.user_id FROM " . USERS_TABLE . " u
                					LEFT JOIN " . USER_GROUP_TABLE ." ug ON u.user_id=ug.user_id AND ug.nuke_group_id='$new_nuke_group_id'
                					WHERE u.nuke_user_posts>='$group_count' AND u.nuke_user_posts<'$group_count_max'
                					AND ug.nuke_group_id is NULL
                					AND u.user_id NOT IN ('$group_moderator','".ANONYMOUS."')";
                				if ( !($result = $db->sql_query($sql)) )
                				...
                				{
                					$sql = "INSERT INTO " . USER_GROUP_TABLE . " (nuke_group_id, user_id, user_pending) 
                						VALUES ($new_nuke_group_id, " . $new_members['user_id'] . ", 0)";
                					if ( !($result2 = $db->sql_query($sql)) )
                					{
                						message_die(GENERAL_ERROR, 'Error inserting user group, group count', '', __LINE__, __FILE__, $sql);
                					}
 ...
 ******************************************************/
                        if(isset($_POST['initial_group'])) {
                            ...
                            $sql = $sql = "UPDATE
                            " . CONFIG_TABLE . "
                            SET config_value = '$new_nuke_group_id'
                            WHERE config_name ='initial_nuke_group_id'";
                            $result = $db->sql_query($sql);
