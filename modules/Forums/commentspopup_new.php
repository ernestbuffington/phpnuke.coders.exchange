...
if($mode == "update")
	...
	$sql = "SELECT username, nuke_user_avatar_type, user_allowavatar, user_avatar FROM " . USERS_TABLE . " WHERE user_id = " . $userinfo['user_id'] ;
	if( !($result = $db->sql_query($sql)) )
	{
	...
	$nuke_user_avatar_type = $row['nuke_user_avatar_type'];
	$user_allowavatar = $row['user_allowavatar'];
	$user_avatar = $row['user_avatar'];
	$avatar_img = '';

	if ( $nuke_user_avatar_type && $user_allowavatar )
	{
