...
$poster_rank = '';
$rank_image = '';
if ( $userdata['nuke_user_rank'] )
{
    for($i = 0; $i < count($ranksrow); $i++)
    {
        if ( $userdata['nuke_user_rank'] == $ranksrow[$i]['rank_id'] && $ranksrow[$i]['rank_special'] )
        {
...
else
{
    for($i = 0; $i < count($ranksrow); $i++)
    {
        if ( $userdata['nuke_user_posts'] >= $ranksrow[$i]['rank_min'] && !$ranksrow[$i]['rank_special'] )
        {
            if (empty($userdata['user_custom_title'])){
            $poster_rank = $ranksrow[$i]['rank_title'];}
            else{$poster_rank = $userdata['user_custom_title'];}
...
// Gets the avatar
$avatar_img = '';
if ( $userdata['nuke_user_avatar_type'] && $userdata['user_allowavatar'] )
{
    switch( $userdata['nuke_user_avatar_type'] )
    {
