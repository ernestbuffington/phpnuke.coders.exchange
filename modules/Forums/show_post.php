...
if($row = $db->sql_fetchrow($result)):
    ...
    do
    {
        ...
        $poster_posts = ( $row['user_id'] != ANONYMOUS ) ? $lang['Posts'] . ': ' . $row['nuke_user_posts'] : '';

        $poster_from = ( $row['user_from'] && $row['user_id'] != ANONYMOUS ) ? $lang['Location'] . ': ' . $row['user_from'] : '';
        $poster_from = str_replace(".gif", "", $poster_from);
        $poster_joined = ( $row['user_id'] != ANONYMOUS ) ? $lang['Joined'] . ': ' . $row['nuke_user_regdate'] : '';

        $poster_avatar = '';
        
		# Mod: View/Disable Avatars/Signatures v1.1.2 START
        if ( $row['nuke_user_avatar_type'] && $poster_id != ANONYMOUS && $row['user_allowavatar'] && $userdata['user_showavatars']):
        ...
        elseif($row['nuke_user_rank']):
            for($j = 0; $j < count($ranksrow); $j++):
                if ( $row['nuke_user_rank'] == $ranksrow[$j]['rank_id'] && $ranksrow[$j]['rank_special']):
            ...
            for($j = 0; $j < count($ranksrow); $j++):
                if( $row['nuke_user_posts'] >= $ranksrow[$j]['rank_min'] && !$ranksrow[$j]['rank_special']):
                    $poster_rank = $ranksrow[$j]['rank_title'];
                    $rank_image = ( $ranksrow[$j]['rank_image'] ) ? '<img src="'.$ranksrow[$j]['rank_image'].'" alt="'.$poster_rank.'" title="'.$poster_rank.'" border="0" /><br />' : '';
                endif;
            endfor;
        ...
        $nuke_user_sig = ( $row['enable_sig'] && !empty($row['nuke_user_sig']) && $board_config['allow_sig'] ) ? $row['nuke_user_sig'] : '';
		# Mod: View/Disable Avatars/Signatures v1.1.2 END

        $nuke_user_sig_bbcode_uid = $row['nuke_user_sig_bbcode_uid'];
