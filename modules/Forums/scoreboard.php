...
if ($total_score) {
        for($i = 0; $i < $total_score; $i++) {
	            ...
	            $user_allowavatar) = $db->sql_ufetchrow("SELECT `user_avatar`,`nuke_user_avatar_type`, `user_allowavatar` FROM `".$prefix."_users` WHERE `user_id`=".$score_rowset[$i]['user_id']."", SQL_NUM);
