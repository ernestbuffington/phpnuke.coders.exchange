...
while($row = $db->sql_fetchrow($result)):
  $moderators[] = '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=".$row['nuke_group_id']).'">'.GroupColor($row['group_name']).'</a>';
endwhile;
...
if($total_topics):
   for($i = 0; $i < $total_topics; $i++):
	...
	$icon = get_icon_title($topic_rowset[$i]['topic_icon'] ?? 0, 1, $type);
	$icon_ID = $topic_rowset[$i]['topic_icon'] ?? 0;
    # Mod: Post Icons v1.0.1 END
...
if($total_forums)
{
	...
	while($row = $db->sql_fetchrow($result)):
		$subforum_moderators[$row['forum_id']][] = '<a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=" . $row['nuke_group_id']) . '">' . 	GroupColor($row['group_name']) . '</a>';
	endwhile;
	$db->sql_freeresult($result);
