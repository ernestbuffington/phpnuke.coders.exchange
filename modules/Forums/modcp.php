 ...
 ******************************************************/
                                ...
                                $count_sql[] = "UPDATE " . USERS_TABLE . "
                                        ...
                                        SET nuke_user_posts = nuke_user_posts - " . $row['posts'] . ", user_reputation = user_reputation - " . $rep . "
...
/*****[BEGIN]******************************************
 [ Mod:    Auto Group                          v1.2.2 ]
 ******************************************************/
                                $user_updated [] = "SELECT ug.user_id, g.nuke_group_id as g_id, u.nuke_user_posts, g.group_count, g.group_count_max, ".$row['poster_id']." as u_id FROM (" . GROUPS_TABLE . " g, ".USERS_TABLE." u)

                                        LEFT JOIN ". USER_GROUP_TABLE." ug ON g.nuke_group_id=ug.nuke_group_id AND ug.user_id=".$row['poster_id']."
                        ...
                        if ( sizeof($user_updated) )
                        {
                        	for($i = 0; $i < sizeof($user_updated); $i++)
                        	{
                        		...
                        		while ($group_data = $db->sql_fetchrow($result))
                        		{
                        			...
                        			$user_add = ($group_data['group_count'] == $group_data['nuke_user_posts'] && $group_data['u_id']!=ANONYMOUS) ? TRUE : FALSE;

                        			$user_remove = ($group_data['group_count'] > $group_data['nuke_user_posts'] && $group_data['u_id']!=ANONYMOUS) ? TRUE : FALSE;
