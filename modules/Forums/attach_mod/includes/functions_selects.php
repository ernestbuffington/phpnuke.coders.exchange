...
function group_select($select_name, $default_group = 0)
{
    ...
    if ($num_rows > 0)
    {
        $group_name[$num_rows]['nuke_group_id'] = 0;
        ...
        for ($i = 0; $i < sizeof($group_name); $i++)
        {
            ...
            {
                $selected = ($group_name[$i]['nuke_group_id'] == $default_group) ? ' selected="selected"' : '';
            }

            $group_select .= '<option value="' . $group_name[$i]['nuke_group_id'] . '"' . $selected . '>' . $group_name[$i]['group_name'] . '</option>';
        }
    }
...
*/
function download_select($select_name, $nuke_group_id = 0)
{
...
*/
function category_select($select_name, $nuke_group_id = 0)
{
    global $db, $types_category, $modes_category;

    $sql = 'SELECT nuke_group_id, cat_id
        FROM ' . EXTENSION_GROUPS_TABLE;
    ...
    if ($num_rows > 0)
    {
        ...
        {
            if ($nuke_group_id == $rows[$i]['nuke_group_id'])
            {
