...
function process_quota_settings($mode, $id, $quota_type, $quota_limit_id = 0)
{
    ...
    if ($mode == 'user')
    {
        ...
        else
        {
            ...
            {
                $sql_ary = ['user_id'        => (int) $id, 'nuke_group_id'        => 0, 'quota_type'    => (int) $quota_type, 'quota_limit_id'=> (int) $quota_limit_id];
    ...
    else if ($mode == 'group')
    {
        ...
        else
        {
            ...
            if ($db->sql_numrows($result) == 0)
            {
                $sql = 'INSERT INTO ' . QUOTA_TABLE . " (user_id, nuke_group_id, quota_type, quota_limit_id) 
                    VALUES (0, $id, $quota_type, $quota_limit_id)";
            }
            else
            {
                $sql = 'UPDATE ' . QUOTA_TABLE . " SET quota_limit_id = $quota_limit_id 
                    WHERE nuke_group_id = $id AND quota_type = $quota_type";
            }
