...
function attachment_quota_settings(string $admin_mode, string $mode, bool $submit = false) # to this
{
    ...
    if ($admin_mode == 'group' && !$submit && isset($_POST['edit']))
    {
        ...
        $nuke_group_id = get_var(POST_GROUPS_URL, 0);

        // Show the contents
        $sql = 'SELECT quota_limit_id, quota_type FROM ' . QUOTA_TABLE . '
            WHERE nuke_group_id = ' . (int) $nuke_group_id;
    ...
    if ($admin_mode == 'group' && $submit && isset($_POST['group_delete'])) {
        $nuke_group_id = get_var(POST_GROUPS_URL, 0);
        process_quota_settings($admin_mode, $nuke_group_id, QUOTA_UPLOAD_LIMIT, 0);
        process_quota_settings($admin_mode, $nuke_group_id, QUOTA_PM_LIMIT, 0);
    } elseif ($admin_mode == 'group' && $submit) {
        $nuke_group_id = get_var(POST_GROUPS_URL, 0);
        // Get the contents
        $upload_quota = get_var('group_upload_quota', 0);
        $pm_quota = get_var('group_pm_quota', 0);
        process_quota_settings($admin_mode, $nuke_group_id, QUOTA_UPLOAD_LIMIT, $upload_quota);
        process_quota_settings($admin_mode, $nuke_group_id, QUOTA_PM_LIMIT, $pm_quota);
    }
...
*/
function display_upload_attach_box_limits($user_id, $nuke_group_id = 0)
{
    ...
    if ($nuke_group_id && !user_in_group($user_id, $nuke_group_id))
    {
        return;
    }
    ...
    if ($nuke_group_id !== 0) {
        $sql = 'SELECT l.quota_limit
            ...
            WHERE q.nuke_group_id = ' . (int) $nuke_group_id . '
