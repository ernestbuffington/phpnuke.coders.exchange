...
for ($i=0; $i < count($spe_ranks); $i++ )
{
    ...
    // base sql request
    // $sql_base = "SELECT * FROM " . USERS_TABLE . " WHERE user_active = 1 AND nuke_user_rank = $rank ORDER BY username";
    $sql_base = "SELECT * FROM " . USERS_TABLE . " WHERE user_active = 1 AND nuke_user_rank = $rank OR user_rank2 = $rank OR user_rank3 = $rank OR user_rank4 = $rank OR user_rank5 = $rank ORDER BY username";
 ...
 ******************************************************/
            ...
            {
                $spe_ranks[$i]['users_list'] .= ($spe_ranks[$i]['users_list'] == '') ? '' : ', ';
                $spe_ranks[$i]['users_list'] .= ( !$profilcp ) ? '...' : '<a href="' . "modules.php?name=Profile&amp;mode=buddy&amp;sub=memberlist&amp;filter=nuke_user_rank_title&amp;comp=eq&amp;fvalue=$rank_title" . '">...</a>';
            }
        }
    }
...
for ($i=count($ranks)-1; $i >=0; $i--)
{
    ...
    // count users
    // $sql_base = "SELECT * FROM " . USERS_TABLE . " WHERE user_active = 1 AND (nuke_user_rank = 0 OR nuke_user_rank IS NULL) AND nuke_user_posts >= $rank_min" . (($rank_max < 99999999)  ? " AND nuke_user_posts < $rank_max" : "" );
    $sql_base = "SELECT * FROM " . USERS_TABLE . " WHERE user_active = 1 AND (nuke_user_rank = 0 OR nuke_user_rank IS NULL OR user_rank2 = 0 OR user_rank2 IS NULL OR user_rank3 = 0 OR user_rank3 IS NULL OR user_rank4 = 0 OR user_rank4 IS NULL OR user_rank5 = 0 OR user_rank5 IS NULL) AND nuke_user_posts >= $rank_min" . (($rank_max < 99999999)  ? " AND nuke_user_posts < $rank_max" : "" );
 ...
 ******************************************************/
            ...
            {
                $ranks[$i]['users_list'] .= ($ranks[$i]['users_list'] == '') ? '' : ', ';
                $ranks[$i]['users_list'] .= ( !$profilcp ) ? '...' : '<a href="' . "modules.php?name=Profile&amp;mode=buddy&amp;sub=memberlist&amp;filter=nuke_user_rank_title&amp;comp=eq&amp;fvalue=$rank_title" . '">...</a>';
            }
        }
    }
