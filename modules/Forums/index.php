...
{
        $sql = "SELECT f.*, p.post_time, p.post_username, u.username, u.user_id, u.user_avatar, u.nuke_user_avatar_type
                FROM (( " . FORUMS_TABLE . " f
  ...
  if ( ($total_forums = count($forum_data)) === 0 )
  {
    ...
    if (isset($userdata['session_logged_in']))
    {
        ...
        while( $row = $db->sql_fetchrow($result) ) 
        {
		  ...
		  $forum_moderators[$row['forum_id']][] = '<a style="font-size:0.9em !important;" href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=".$row['nuke_group_id']).'">'.GroupColor($row['group_name']).'</a>';
        }
        $db->sql_freeresult($result);
                                ...
                                for($j = 0; $j < $total_forums; $j++)
                                {
                                        if ( $forum_data[$j]['cat_id'] == $cat_id )
                                        {
                                            ...
                                            ******************************************************/
                                            ...
                                            switch( $forum_data[$j]['nuke_user_avatar_type'] )
                                            {
