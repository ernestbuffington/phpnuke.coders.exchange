...
$leave_out['show_sig_once'] = false;
    ...
    $poster_posts = ( $postrow[$i]['user_id'] != ANONYMOUS ) ? $postrow[$i]['nuke_user_posts'] : '';
    ...
    # Mod: Member Country Flags v2.0.7 END
    $poster_joined = ( $postrow[$i]['user_id'] != ANONYMOUS ) ? $postrow[$i]['nuke_user_regdate'] : '';
    ...
    # Mod: Display Poster Information Once v2.0.0 START
    if($postrow[$i]['nuke_user_avatar_type'] && $poster_id != ANONYMOUS && $postrow[$i]['user_allowavatar'] && $userdata['user_showavatars'] && !$leave_out['show_avatar_once']):
    ...
    # Mod: Default avatar v1.1.0 END
		...
		$search_img = '<a href="'.$temp_url.'"><img src="'.$images['icon_search'].'" alt="'.sprintf($lang['Search_nuke_user_posts'], $postrow[$i]['username']).'" 
		title="'.sprintf($lang['Search_nuke_user_posts'], $postrow[$i]['username']).'" border="0" /></a>';

		$search = '<a href="'.$temp_url.'">'.sprintf($lang['Search_nuke_user_posts'], $postrow[$i]['username']).'</a>';
        $search_alt = sprintf($lang['Search_nuke_user_posts'], $postrow[$i]['username']);
        ...
        $nuke_user_sig = ($postrow[$i]['enable_sig'] && !empty($postrow[$i]['nuke_user_sig']) && $board_config['allow_sig'] ) ? $postrow[$i]['nuke_user_sig'] : '';
        # Mod: View/Disable Avatars/Signatures v1.1.2 END
        ...
        if ($display_ad):
	      ...
	      if ($board_config['ad_no_groups'] != ''):
              ...
              $sql = "SELECT 1
                  ...
                  WHERE user_id=".$userdata['user_id']." AND (nuke_group_id=0";
		      for ($a=0; $a < count($ad_no_groups); $a++):
              $sql .= " OR nuke_group_id=".$ad_no_groups[$a];
              endfor;
              $sql .= ")";
		      if(!($result = $db->sql_query($sql)))
		  ...
		  if ($userdata['user_id'] != ANONYMOUS && ($board_config['ad_post_threshold'] != '') && ($userdata['nuke_user_posts'] >= $board_config['ad_post_threshold']))
          $display_ad = false;
		  ...
		  if (isset($_GET['directed']) && $_GET['directed'] == 'ftr'):
			...
			if ($active == 1):
				...
				if($who == 1):
					...
					if ($userdata['nuke_user_regdate'] > $installed):
