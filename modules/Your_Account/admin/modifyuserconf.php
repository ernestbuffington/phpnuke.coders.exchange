...
if(is_mod_admin($module_name)) {
    ...
    $chng_nuke_user_sig = ya_fixtext($chng_nuke_user_sig);
    ...
    if (empty($stop)) 
    {
        if (!empty($chng_pass)) 
        {
            ...
            /*****[END]********************************************
             [ Base:     Evolution Functions               v1.5.0 ]
             ******************************************************/
            $db->sql_query("UPDATE ".$user_prefix."_users SET username='$chng_uname', name='$chng_name', user_email='$chng_email', femail='$chng_femail', user_website='$chng_url', user_from='$chng_user_from', user_occ='$chng_user_occ', user_interests='$chng_user_interests', user_viewemail='$chng_user_viewemail', user_avatar='$chng_avatar', nuke_user_sig='$chng_nuke_user_sig', user_password='$cpass', newsletter='$chng_newsletter' WHERE user_id='$chng_uid'");
            if ($Version_Num > 6.9) { $db->sql_query("UPDATE ".$user_prefix."_users SET points='$chng_points' WHERE user_id='$chng_uid'"); }
        } else {
            $db->sql_query("UPDATE ".$user_prefix."_users SET username='$chng_uname', name='$chng_name', user_email='$chng_email', femail='$chng_femail', user_website='$chng_url', user_from='$chng_user_from', user_occ='$chng_user_occ', user_interests='$chng_user_interests', user_viewemail='$chng_user_viewemail', user_avatar='$chng_avatar', nuke_user_sig='$chng_nuke_user_sig', newsletter='$chng_newsletter' WHERE user_id='$chng_uid'");
            if ($Version_Num > 6.9) { $db->sql_query("UPDATE ".$user_prefix."_users SET points='$chng_points' WHERE user_id='$chng_uid'"); }
        }
