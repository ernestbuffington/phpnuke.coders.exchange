...
if(is_mod_admin($module_name)) {
    ...
    if ($add_email != $add_email2) {
        ...
        OpenTable();
        echo "<div align=\"center\"><span class='title'><strong>"._ERRORREG."</strong></span><br /><br />";
        echo "<span class='content'>"._EMAILDIFFERENT."<br /><br />"._GOBACK."</span></div>";
        CloseTable();
    ...
    $add_nuke_user_sig = str_replace("<br />", "\r\n", $add_nuke_user_sig);
    $add_nuke_user_sig = ya_fixtext($add_nuke_user_sig);
    ...
    if (empty($stop)) {
        ...
        $sql .= "(user_id, name, username, user_email, femail, user_website, nuke_user_regdate, user_from, user_occ, user_interests, user_viewemail, user_avatar, nuke_user_avatar_type, nuke_user_sig, user_password, newsletter, broadcast, popmeson";
        
		//if ($Version_Num > 6.9) { $sql .= ", points"; }
        $sql .= ") ";
        $sql .= "VALUES ('$new_uid', '$add_name', '$add_uname', '$add_email', '$add_femail', '$add_url', '$nuke_user_regdate', '$add_user_from', '$add_user_occ', '$add_user_intrest', '$add_user_viewemail', 'gallery/blank.png', '3', '$add_nuke_user_sig', '$add_pass', '$add_newsletter', '1', '0'";
        ...
        if (!$result) {
            ...
            OpenTable();
            echo "<div align=\"center\"><strong>"._ERRORSQL."</strong></div>";
            CloseTable();
            include_once(NUKE_BASE_DIR.'footer.php');
            return;
