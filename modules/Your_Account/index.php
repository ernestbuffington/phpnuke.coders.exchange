...
if (isset($username) && (preg_match('#[^a-zA-Z0-9_\-]#m',$username))) {
...
function userCheck($username, $user_email) {
	...
	if ((!$user_email) || (empty($user_email)) || (!preg_match('#^[_\\\\\.0-9a-z\-]+@([0-9a-z][0-9a-z\-]+\.)+[a-z]{2,6}$#mi',$user_email))) {
	  $stop = "<div align=\"center\">"._ERRORINVEMAIL."</div><br>";
	}
	
	if (strrpos($user_email,' ') > 0) {
	  $stop = "<div align=\"center\">"._ERROREMAILSPACES."</div>";
	}
	
	if ((!$username) || (empty($username)) || (preg_match('#[^a-zA-Z0-9_\-]#m',$username))) {
	  $stop = "<div align=\"center\">"._ERRORINVNICK."</div><br>";
	}
	
	if (strlen($username) > 25) {
	  $stop = "<div align=\"center\">"._NICK2LONG."</div>";
	}
	
	if (preg_match('#^((root)|(adm)|(linux)|(webmaster)|(admin)|(god)|(administrator)|(administrador)|(nobody)|(anonymous)|(anonimo)|(anónimo)|(operator)|(JackFromWales4u2))$#mi',$username)) {
	  $stop = "<div align=\"center\">"._NAMERESERVED."</div>";
	}

	if (strrpos($username,' ') > 0) {
	  $stop = "<div align=\"center\">"._NICKNOSPACES."</div>";
	}

	if ($db->sql_numrows($db->sql_query("SELECT username FROM ".$user_prefix."_users WHERE username='$username'")) > 0) {
	  $stop = "<div align=\"center\">"._NICKTAKEN."</div><br>";
	}
	...
	if ($db->sql_numrows($db->sql_query("SELECT user_email FROM ".$user_prefix."_users WHERE user_email='$user_email'")) > 0) {
	  $stop = "<div align=\"center\">"._EMAILREGISTERED."</div><br>";
	}
...
function confirmNewUser($username, $user_email, $user_password, $user_password2, $random_num, $gfx_check) {
	...
	if (!$stop) {
		...
		if (extension_loaded("gd") AND $code != $gfx_check AND ($gfx_chk == 3 OR $gfx_chk == 4 OR $gfx_chk == 6 OR $gfx_chk == 7)) {
			...
			OpenTable();
			echo "<div align=\"center\"><strong>"._SECCODEINCOR."</strong><br><br>"
			.""._GOBACK."</div>";
			CloseTable();
		...
		if (empty($user_password) AND empty($user_password2)) {
			...
			OpenTable();
			echo "<div align=\"center\"><strong>"._PASSDIFFERENT."</strong><br><br>"._GOBACK."</div>";
			CloseTable();
			...
			OpenTable();
			echo "<div align=\"center\">"._YOUPASSMUSTBE." <strong>$minpass</strong> "._CHARLONG."<br><br>"._GOBACK."</div>";
			CloseTable();
   ...
   global $stop, $EditedMessage, $adminmail, $sitename, $Default_Theme, $user_prefix, $db, $storyhome, $module_name, $nukeurl;
	...
	if (!isset($stop)) {
		...
		$result = $db->sql_query("INSERT INTO ".$user_prefix."_users_temp (user_id, 
																 ...
																 nuke_user_regdate, 
																         check_num, 
																		      time) VALUES (NULL, '$username', '$user_email', '$new_password', '$nuke_user_regdate', '$check_num', '$time')");
		...
		if(!$result) {
			...
			OpenTable();
			echo "<div align=\"center\"><strong>"._ACCOUNTCREATED."</strong><br><br>";
			echo ""._YOUAREREGISTERED.""
			."<br><br>"
			.""._FINISHUSERCONF."<br><br>"
			.""._THANKSUSER." $sitename!</div>";
			CloseTable();
...
function activate($username, $check_num) {
	...
	if ($db->sql_numrows($result) == 1) {
		...
		if ($check_num == $row['check_num']) {
			...
			$db->sql_query("INSERT INTO ".$user_prefix."_users (user_id, 
												  ...
												  nuke_user_avatar_type, 
												      nuke_user_regdate, 
													     nuke_user_lang) VALUES (NULL, 
														       '".$row['username']."', 
															 '".$row['user_email']."', 
															         '$user_password', 
																  'gallery/blank.gif', 
																                  '3', 
													  '".$row['nuke_user_regdate']."', 
													                      '$language')");
			...
			OpenTable();
			echo "<div align=\"center\"><strong>".$row['username'].":</strong> "._ACTMSG."</div>";
			CloseTable();
			...
			OpenTable();
			echo "<div align=\"center\">"._ACTERROR1."</div>";
			CloseTable();
		...
		OpenTable();
		echo "<div align=\"center\">"._ACTERROR2."</div>";
		CloseTable();
...
function userinfo($username, $bypass=0, $hid=0, $url=0) {
	...
	if((isset($cookie[1])) AND (strtolower($username) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		echo "<span class=\"option\">".htmlentities($username).", "._WELCOMETO." $sitename!</span><br><br>";
		echo "<span class=\"content\">"._THISISYOURPAGE."</span></div><br><br>";
		...
		echo "<span class=\"title\">"._PERSONALINFO.": ".htmlentities($username)."</span></div><br><br>";
	}
	...
	if(($num == 1) && ($userinfo['user_website'] || 
		...
		if ($userinfo['bio']) { 
		   echo "<br><strong>"._EXTRAINFO.":</strong><br>".$userinfo['bio']."<br>\n"; 
		}
		...
		echo "<br><br>"._USERSTATUS.": <strong>$online</strong><br>\n";

		if (($userinfo['newsletter'] == 1) 
		    AND ($username == $cookie[1]) 
		    AND ($userinfo['user_password'] == $cookie[2]) 
		    OR (is_admin($admin) 
		    AND ($userinfo['newsletter'] == 1))) {

			echo "<i>"._SUBSCRIBED."</i><br>";

		} elseif ((isset($cookie[1])) 
		    AND ($userinfo['newsletter'] == 0) 
			AND ($username == $cookie[1]) 
			AND ($userinfo['user_password'] == $cookie[2]) 
			OR (is_admin($admin) 
			AND ($userinfo['newsletter'] == 0))) {
		...
		if (is_user($user) 
			...
			echo ""._YOUHAVEPOINTS." <strong>$n_points</strong><br>";
			...
			if (paid()) {
				...
				if (!empty($subscription_url)) {
					$content = "<br><div align=\"center\">"._YOUARE." <a href='$subscription_url'>"._SUBSCRIBER."</a> "._OF." $sitename<br>";
				} else {
					$content = "<br><div align=\"center\">"._YOUARE." "._SUBSCRIBER." "._OF." $sitename<br>";
				}
				...
				$content .= "<strong>"._SUBEXPIREIN."<br><span style=\"color:#FF000;\">$rest</span></strong></div>";

			} else {

				if (!empty($subscription_url)) {
					$content .= "<br><div align=\"center\">"._NOTSUB." $sitename. "._SUBFROM." <a href='$subscription_url'>"._HERE."</a> "._NOW."";
				} else {
					$content .= "<br><div align=\"center\">"._NOTSUB." $sitename.";
				}
			}
			echo "$content<br><br>";
			...
			if (is_admin($admin)) {
				...
				if ($subnum != 0) {
					...
					$content = "<strong>"._ADMSUBEXPIREIN."<br><span style=\"color:#FF0000;\">$rest</span></strong><br><br>";

					echo "$content";

				} else {
					echo "<div align=\"center\"><strong>"._ADMNOTSUB."</strong><br><br>";
			}
		  }
		}
		...
		echo "<div align=\"center\">"._NOINFOFOR." ".htmlentities($username)."</div>";
	}
	CloseTable();
	...
	if ((isset($cookie[1])) 
		...
		if ($hid != 0 OR ($hid == 0 AND $url != "0" AND $url != "http://") AND !empty($url)) {
			...
			if (!$fp) {
				$content = "<div align=\"center\"><span class=\"content\">"._RSSPROBLEM."</span></div>";
			}
			...
			if ($fp) {
				...
				for ($i=0;$i<10;$i++) {
					...
					if (empty($items[$i]) AND $cont != 1) {
						$content = "<div align=\"center\">"._RSSPROBLEM."</div>";
					} else {
			...
			if (!empty($content)) {
				...
				OpenTable2();
				echo "<div align=\"center\"><strong>"._HEADLINESFROM." <a href=\"http://$siteurl[0]\" target=\"new\">$title</a></strong></div><br>";
				echo "$content";
				CloseTable2();
				...
				OpenTable2();
				echo "<div align=\"center\">"._RSSPROBLEM."</div><br>";
				CloseTable2();
			}
	...
	if ((isset($cookie[1])) AND $broadcast_msg == 1 AND ($username == $cookie[1]) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		OpenTable();
		echo "<div align=\"center\"><strong>"._BROADCAST."</strong><br><br>"._BROADCASTTEXT."<br><br>"
		."<form action=\"modules.php?name=$module_name\" method=\"post\">"
	...
	if ((isset($cookie[1])) 
		...
		OpenTable();
		echo "<div align=\"center\"><strong>"._PRIVATEMESSAGES."</strong><br><br>";
		...
		echo ""._YOUHAVE." <a href=\"modules.php?name=Private_Messages\"><strong>$numrow</strong></a> "._PRIVATEMSG."<br><br>"
	...
	if ($articlecomm == 1) {
		...
		OpenTable();
		echo "<strong>"._LAST10COMMENTS." ".$userinfo['username'].":</strong><br>";
	...
	OpenTable();
	echo "<strong>"._LAST10SUBMISSIONS." ".$userinfo['username'].":</strong><br>";
...
function main($user) {
	...
	if(!is_user($user)) {
		...
		if (!is_user($user)) {
			...
			."<div align=\"center\"><span class=\"content\">[ <a href=\"modules.php?name=$module_name&amp;op=pass_lost\">"._PASSWORDLOST."</a> | <a 
			href=\"modules.php?name=$module_name&amp;op=new_user\">"._REGNEWUSER."</a> ]</span></div>\n";
			CloseTable();
...
function new_user() {
	...
	if (!is_user($user)) {
		...
		OpenTable();
		echo "<form action=\"modules.php?name=$module_name\" method=\"post\">\n"
		."<strong>"._REGNEWUSER."</strong> ("._ALLREQUIRED.")<br><br>\n"
		...
		.""._REGISTERNOW."<br>\n"
		.""._WEDONTGIVE."<br><br>\n"
		."<div align=\"center\"><span class=\"content\">[ <a href=\"modules.php?name=$module_name\">"._USERLOGIN."</a> | <a href=\"modules.php?name=$module_name&amp;op=pass_lost\">"._PASSWORDLOST."</a> ]</span></div>\n";
		CloseTable();
...
function pass_lost() {
	...
	if (!is_user($user)) {
		...
		OpenTable();
		echo "<strong>"._PASSWORDLOST."</strong><br><br>\n"
		.""._NOPROBLEM."<br><br>\n"
		."<form action=\"modules.php?name=$module_name\" method=\"post\">\n"
		."<table border=\"0\"><tr><td>\n"
		.""._NICKNAME.":</td><td><input type=\"text\" name=\"username\" size=\"15\" maxlength=\"25\"></td></tr>\n"
		...
		."<div align=\"center\"><span class=\"content\">[ <a href=\"modules.php?name=$module_name\">"._USERLOGIN."</a> | <a href=\"modules.php?name=$module_name&amp;op=new_user\">"._REGNEWUSER."</a> ]</span></div>\n";
		CloseTable();
...
function logout() {
	...
	echo "<div align=\"center\"><span class=\"option\"><strong>"._YOUARELOGGEDOUT."</strong></span></div>";
	CloseTable();
...
function mail_password($username, $code) {
	...
	if($db->sql_numrows($result) == 0) {
		...
		OpenTable();
		echo "<div align=\"center\">"._SORRYNOUSERINFO."</div>";
		CloseTable();
		...
		if ($areyou==$code) {
			...
			OpenTable();
			echo "<div align=\"center\">"._PASSWORD4." $username "._MAILED."<br><br>"._GOBACK."</div>";
			CloseTable();
			...
			if($db->sql_numrows($result) == 0) {
				...
				OpenTable();
				echo "<div align=\"center\">"._CODEFOR." $username "._MAILED."<br><br>"._GOBACK."</div>";
				CloseTable();
...
function docookie($setuid, $setusername, $setpass, $setstorynum, $setumode, $setuorder, $setthold, $setnoscore, $setublockon, $settheme, $setcommentmax) {
	...
	setcookie("user","$info",['expires' => time()+2592000]);
}
...
function login($username, $user_password, $redirect, $mode, $f, $t, $random_num, $gfx_check) {
	...
	if (preg_match('#privmsg#m', $forward)) {
		$pm_login = "active";
	}
...
function edituser() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		echo "<tr><td bgcolor='$bgcolor2'><strong>"._SIGNATURE.":</strong><br>"._OPTIONAL."</td>"
		."<td bgcolor='$bgcolor3'><textarea wrap=\"virtual\" cols=\"70\" rows=\"15\" name=\"nuke_user_sig\">".$userinfo['nuke_user_sig']."</textarea><br>"._255CHARMAX."</td></tr>"
		."<tr><td bgcolor='$bgcolor2'><strong>"._EXTRAINFO.":</strong><br>"._OPTIONAL."</td>"
		."<td bgcolor='$bgcolor3'><textarea wrap=\"virtual\" cols=\"70\" rows=\"15\" name=\"bio\">".$userinfo['bio']."</textarea><br>"._CANKNOWABOUT."</td></tr>"
		."<tr><td bgcolor='$bgcolor2'><strong>"._PASSWORD."</strong>:</td><br>"
		...
		."<br><strong><h5>Avatar control panel</h5></strong>"
		...
		if ($board_config['allow_avatar_local']) {
			...
			."<tr><td bgcolor='$bgcolor2'><strong>Select Avatar from gallery:</strong></td>"
			...
			echo "<tr><td bgcolor='$bgcolor2'><strong>Select Avatar from gallery:</strong></td>"
			."<td bgcolor='$bgcolor3'><strong>Gallery Avatars Currently Disabled</strong></td></tr>";
		}

		if ($board_config['allow_avatar_upload']) {

			echo "<tr><td bgcolor='$bgcolor2'><strong>Upload Avatar from your machine:</strong></td>"
			."<td bgcolor='$bgcolor3'><a href=\"modules.php?name=Forums&file=profile&mode=editprofile\"><strong>Upload Through Forum Profile</strong></a></td></tr>"
		...
		if ($board_config['allow_avatar_remote']) {
			...
			echo "<tr><td bgcolor='$bgcolor2'><strong>Link to off-site Avatar:</strong><br><span class=gensmall>Enter the URL of the location containing the Avatar image you wish to link to and click on the submit button below.</span></td>"
			."<td bgcolor='$bgcolor3'><strong>Currently Disabled</strong></td></tr>";
		}
...
function saveuser($realname, 
	...
	$nuke_user_sig = filter($nuke_user_sig, "", 1);
	...
	if (($user_id == $vuid) AND ($check2 == $ccpass)) {
		...
		if ((isset($user_password)) && ("$user_password" != "$vpass")) {
			echo "<div align=\"center\">"._PASSDIFFERENT."</div>";
		} elseif ((!empty($user_password)) && (strlen($user_password) < $minpass)) {
			echo "<div align=\"center\">"._YOUPASSMUSTBE." <strong>$minpass</strong> "._CHARLONG."</div>";
		} else {
			...
			if (!empty($user_password)) {
				...
				$db->sql_query("UPDATE ".$user_prefix."_users SET name='$realname', 
									...
									user_allow_viewonline='$user_allow_viewonline', 
										...
										nuke_user_notify_pm='$nuke_user_notify_pm', 
										  ...
										  nuke_user_timezone='$nuke_user_timezone', 
									  nuke_user_dateformat='$nuke_user_dateformat' WHERE user_id='$user_id'");
				...
				if ($db->sql_numrows($result) == 1) {
					...
					echo "<div align=\"center\">"._SOMETHINGWRONG."</div><br>";
				}
...
function edithome() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		OpenTable();
		echo "<div align=\"center\"><span class=\"title\"><strong>"._HOMECONFIG."</strong></span></div>";
		CloseTable();
		...
		." <strong>"._ACTIVATEPERSONAL."</strong>"
		."<br>"._CHECKTHISOPTION.""
...
function chgtheme() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		OpenTable();
		echo "<div align=\"center\"><span class=\"title\"><strong>"._THEMESELECTION."</strong></span></div>";
		CloseTable();
...
function editcomm() {
	...
	if ((is_user($user)) AND (strtolower($userinfo['username']) == strtolower($cookie[1])) AND ($userinfo['user_password'] == $cookie[2])) {
		...
		OpenTable();
		echo "<div align=\"center\"><span class=\"title\"><strong>"._COMMENTSCONFIG."</strong></span></div>";
		CloseTable();
...
function avatarlist($avatarcategory) {
 ...
 global $user, $userinfo, $cookie, $module_name;
	...
	$avatarcatname = preg_replace ('#_#m', "&nbsp;", $avatarcategory);
