...
if ( ($userdata['nuke_user_posts'] < $pm_allow_threshold) && $userdata['user_level'] != ADMIN)
{
...
if ( $folder == 'inbox' ) 
{
    $max_boxsize_sql = "SELECT ug.nuke_group_id, g.max_inbox, g.override_max_inbox FROM " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = " . $userdata['user_id'] . " AND ug.nuke_group_id = g.nuke_group_id ORDER BY override_max_inbox DESC, max_inbox DESC";
...
{
    $max_boxsize_sql = "SELECT ug.nuke_group_id, g.max_savebox, g.override_max_savebox FROM " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = " . $userdata['user_id'] . " AND ug.nuke_group_id = g.nuke_group_id ORDER BY override_max_savebox DESC, max_savebox DESC";
    $max_boxsize_result = $db->sql_query($max_boxsize_sql);
    $max_boxsize_row = $db->sql_fetchrow($max_boxsize_result);
...
{
    $max_boxsize_sql = "SELECT ug.nuke_group_id, g.max_sentbox, g.override_max_sentbox FROM " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE . " g WHERE ug.user_id = " . $userdata['user_id'] . " AND ug.nuke_group_id = g.nuke_group_id ORDER BY override_max_sentbox DESC, max_sentbox DESC";
    $max_boxsize_result = $db->sql_query($max_boxsize_sql);
    $max_boxsize_row = $db->sql_fetchrow($max_boxsize_result);
 ...
 ******************************************************/
		...
		AS user_id_2, u.nuke_user_sig_bbcode_uid, 
		              u.nuke_user_posts, 
					  u.user_from, 
					  u.user_website, 
					  u.user_birthday, 
 ...
 ******************************************************/
       ...
       ******************************************************/
        ...
        switch( $privmsg['nuke_user_avatar_type'] ):
 ...
 ******************************************************/
	   ...
	   $username_to = '';
		...
		$search_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_search'] . '" alt="' . sprintf($lang['Search_nuke_user_posts'], $username_from) . '" 
		title="' . sprintf($lang['Search_nuke_user_posts'], $username_from) . '" border="0" /></a>';
        
		$search = '<a href="' . $temp_url . '">' . sprintf($lang['Search_nuke_user_posts'], $username_from) . '</a>';
/*****[BEGIN]******************************************
 [ Mod:    Online/Offline/Hidden               v2.2.7 ]
 ******************************************************/
 ...
 ******************************************************/
        ...
        {
                $nuke_user_sig = ( $board_config['allow_bbcode'] ) ? bbencode_second_pass($nuke_user_sig, $nuke_user_sig_bbcode_uid) : preg_replace('/\:[0-9a-z\:]+\]/si', ']', (string) $nuke_user_sig);
        }
...
else if ( $submit || $refresh || !empty($mode) )
{
        ...
        $nuke_user_sig = ( !empty($userdata['nuke_user_sig']) && $board_config['allow_sig'] ) ? $userdata['nuke_user_sig'] : "";
 ...
 ******************************************************/
                        ...
                        $sql = "SELECT user_id, username, nuke_user_notify_pm, user_email, nuke_user_lang, user_active
                                FROM " . USERS_TABLE . "
        ...
        else if ( $preview || $refresh || $error )
        {
                ...
                else if ( $mode == 'edit' )
                {
                        ...
                        if ( $postrow = $db->sql_fetchrow($result) )
                        {
                                ...
                                $nuke_user_sig = ( !empty($postrow['nuke_user_sig']) && $board_config['allow_sig'] ) ? $postrow['nuke_user_sig'] : '';
                        }
                }
        }
        else
...
//$header_location = ( @preg_match('/Microsoft|WebSTAR|Xitami/', $_SERVER['SERVER_SOFTWARE']) ) ? 'Refresh: 0; URL=' : 'Location: ';
        ...
        if ( !$html_on || !$board_config['allow_html'] || !$userdata['user_allowhtml'] )
                ...
                if ( $attach_sig && !empty($nuke_user_sig) && $userdata['nuke_user_sig_bbcode_uid'] )
                {
...
{
    $sql_g = "SELECT DISTINCT g.nuke_group_id
    FROM ".GROUPS_TABLE . " g, ".USER_GROUP_TABLE . " ug
    WHERE g.group_single_user <> 1
        AND (
            (g.group_allow_pm='".AUTH_MOD."' AND g.group_moderator = '" . $userdata['user_id']."') OR
            (g.group_allow_pm='".AUTH_ACL."' AND ug.user_id = " . $userdata['user_id'] . " AND ug.nuke_group_id = g.nuke_group_id ) OR
