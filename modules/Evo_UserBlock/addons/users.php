...
function evouserinfo_new_today() 
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$user_prefix."_users WHERE nuke_user_regdate='".date("M d, Y")."'";
    $result = $db->sql_query($sql);
    $row = $db->sql_fetchrow($result);
    $db->sql_freeresult($result);
...
function evouserinfo_new_yesterday() 
{
    ...
    $sql = "SELECT COUNT(*) FROM ".$user_prefix."_users WHERE nuke_user_regdate='".date("M d, Y", time()-86400)."'";
    $result = $db->sql_query($sql);
    $row = $db->sql_fetchrow($result);
    $db->sql_freeresult($result);
