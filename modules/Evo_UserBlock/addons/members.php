    ...
    if(is_user()) 
	{
	   ...
	   if ($db->sql_numrows($result)) 
	   {
	      ...
	      while( $row = $db->sql_fetchrow($result) ) 
		  {
		    ...
		    $evouserinfo_members .= '<div style="padding-left: 10px;">';
			$evouserinfo_members .= '<font title="'.$row['nuke_group_id'].'" class="tooltip-html-side-interact tooltipstered" 
			color="lightgrey"><i title="'.$row['nuke_group_id'].'" alt="'.$row['nuke_group_id'].'" class="fas fa-users"></i></font> ';
		    $evouserinfo_members .= '<a class="tooltip-html-side-interact tooltipstered" title="'.$row['group_name'].'"href="modules.php?name=Groups&amp;g='.$row['nuke_group_id'] . '"><strong>' . $group_name . '</strong></a><br />';
			$evouserinfo_members .= '</div>';
          }
