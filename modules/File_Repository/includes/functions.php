...
/*
|-----------------------------------------------------------------------
|	COPYRIGHT           :   (c) 2023 by phpnuke.coders.exchange
|	ORIGINAL COPYRIGHT  :   (c) 2016 by lonestar-modules.com
|	AUTHOR(s) 			:	Lonestar, Ernest Buffington, Francisco Burzi	
|	COPYRIGHTS 			:	lonestar-modules.com
|	PROJECT 			:	File Repository
|	VERSION 			:	v1.0.2 PHP-Nuke version Only
|----------------------------------------------------------------------
*/

/*
 * This file has been patched for use with PHP v8.2.4
 */
...
$more_js .= '	var which_cms				= "'.((!defined('NUKE_FILE')) ? 'ravennuke' : 'evolution').'";'."\n";
...
function _category_parents($name,$value=0)
{
	...
	$category_list .= ((!defined('NUKE_FILE')) ? 'padding: 4px; ' : 'padding: 5px; ');
...
function _check_users_group($gid) 
{
	...
	elseif (is_user($user)) 
	{
		...
		$result = $db->sql_query("SELECT COUNT(*) FROM `"._USER_GROUP_TABLE."` WHERE nuke_group_id='".$gid."' AND user_id='".$guid."' AND user_pending != '1'");
...
function _collect_iteminfo($id,$isfile=false)
{
	...
	if($row['author'])
		$iteminfo['author']			= (!defined('NUKE_FILE')) ? $row['author'] : UsernameColor($row['author']);
	else
		$iteminfo['author']			= $lang_new[$module_name]['NA'];
	$iteminfo['author_email'] 	= $row['author_email'];
	
	$iteminfo['author_www'] 	= ($row['author_website']) ? '<a href="http://'.str_replace('http://','',$row['author_website']).'" 
	target="_blank" alt="'.$lang_new[$module_name]['AUTHOR_WEBSITE'].'" title="'.$lang_new[$module_name]['AUTHOR_WEBSITE'].'"><span 
	class="dm-sprite www"></span></a><span style="margin-left: 5px;">'.$iteminfo['author'].'</span>' : 
...
{
	global $db;
	return (!defined('NUKE_FILE')) ? FixQuotes($query) : Fix_Quotes($query);
}
...
function _list_available_permission_groups($name, $value)
{
	...
	$groups .= ((!defined('NUKE_FILE')) ? 'padding: 4px; ' : 'padding: 5px; ');
	...
	if( $db->sql_numrows($result) > 0 )
	{
		...
		{
			$ginfo['nuke_group_id'] = $ginfo['nuke_group_id'] + 2;
			$groups .= '  <option value="'.$ginfo['nuke_group_id'].'" '.(($value == $ginfo['nuke_group_id']) ? 'selected="selected"' : '').'>'._sut($ginfo['group_name']).'</option>'."\n";
		}
		$db->sql_freeresult($result);
	}
	$groups .= '      </select>'."\n";
...
function _selectbox($n,$ops,$v,$r=false)
{
	...
	$select .= (!defined('NUKE_FILE')) ? 'padding: 4px; ' : 'padding: 5px; ';
...
function _user_is_within_group_name($gid,$isallowed=true) 
{
    ...
    else
    {
		...
		{
			$nuke_group_id = ($gid - 2);
			list($group_name) = $db->sql_fetchrow($db->sql_query("SELECT `group_name` FROM `"._GROUPS_TABLE."` WHERE `nuke_group_id`='$nuke_group_id'"));
			$groupname = ((!defined('NUKE_FILE')) ? _sut($group_name) : _sut(GroupColor($group_name)));
		}
	}
	return $groupname;
}
