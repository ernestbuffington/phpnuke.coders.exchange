...
//---------------------------------------------------------------------
		if($userinfo['nuke_user_posts'] <= $iteminfo['posts'] && $iteminfo['posts'] <> 0)
		{
			echo '  <tr'._bgColor(2).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sh(),3).'>'._suh($lang_new[$module_name]['ERROR']).'</td>'."\n";
			echo '  </tr>'."\n";
			echo '  <tr'._bgColor(1).'>'."\n";
			echo '    <td'._tdcss(false,'center',_sc(),3).'>'.sprintf($lang_new[$module_name]['POSTS_REQUIRED'],$userinfo['nuke_user_posts'],$iteminfo['posts']).'</td>'."\n";
			echo '  </tr>'."\n";
		}
...
//---------------------------------------------------------------------
		...
		elseif($iteminfo['groups'] > 0 && _check_users_permissions($iteminfo['groups']) == true && $userinfo['nuke_user_posts'] >= $iteminfo['posts'])
		{
			$submitButton = _submit(sprintf($lang_new[$module_name]['DOWNLOAD_ITEM'],$iteminfo['title']));
		}
		else
...
function _retrieve_files()
{
	...
	// $gfxcheck 	= (!defined('NUKE_FILE')) ? security_code_check($_POST['gfx_check'],array(2,4,5,7)) : security_code_check($_POST['g-recaptcha-response'],array(2,4,5,7));
	$adminBypass = ($settings['adminBypass'] == true && is_admin($admin)) ? false : true;
