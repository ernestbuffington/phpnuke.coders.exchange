	...
	if($numrows > 0)
	{
		...
		else 
		{
			...
			while($d = $db->sql_fetchrow($result))
			{
				...
				echo '<td'._tdcss('10%','center',_sc()).'><div class="thumbnail_border"><a'._image_viewer($screen[$items]['filename'],'item-'.$d['did']).' 
				href="'._FILE_REPOSITORY_SCREENS.$screen[$items]['filename'].'" title="'.$screen[$items]['title'].'"><img 
				src="'._FILE_REPOSITORY_SCREENS.'thumbs/thumb_100x100_'.$screen[$items]['filename'].'" border="0" /></a></div></td>';
