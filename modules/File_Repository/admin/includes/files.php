...
function _file_repository_files()
{
	...
	while($download = $db->sql_fetchrow($result))
	{
		if($download['title'])
		{
			...
			echo '<td'._tdcss('80%',FALSE).'><a'._ls().' href="modules.php?name='.$module_name.'&amp;action=view&amp;did='.$download['did'].'" 
			target="_blank">'._colorization($download['title'],$download['color']).'</a></td>'."\n";
...
function _file_repository_broken_files()
{
	...
	if($count_broken_downloads['isbroken'] > 0):
		...
		while($broken = $db->sql_fetchrow($result)):
			...
			echo '<td'._tdcss('20%','center',_sc()).'><a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=editfile&amp;did='.$broken['did'].'">'.$lang_new[$module_name]['REPAIR'].'</a>&nbsp;|&nbsp;<a 
			href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=deleteitem&amp;did='.$broken['did'].'">'.$lang_new[$module_name]['DELETE'].'</a></td>'."\n";
...
function _file_repository_new_file()
{
	...
	echo '<td'._tdcss('50%',FALSE,_sc()).'><a'._ls().' href="javascript:void(0);" class="mainforum_btn" id="isuploaded"><span 
	style="text-transform: '.(($settings['utext'] == 1) ? 'uppercase' : 'none').';">'.$lang_new[$module_name]['SHOW_LOCAL'].'</span></a></td>'."\n";
	...
	while (false !== ($filename = readdir($dh))) 
	{
		...
		if($filename != '.' && $filename != '..' && $filename != '.htaccess' && $filename != 'index.html' && $filename != 'screenshots' && $count != 1)
		{
			...
			echo '<td'._tdcss('25%').'><input data-titleid="'.$i.'" type="text" class="atitle" name="atitle'.$i.'" value="" 
			autocomplete="off" style="border: 1px solid; box-sizing: border-box; font-size: 13px; letter-spacing: 1px; margin: 0px 1px 1px; padding: 5px; width: 94%;"></td>';
			
			echo '<td'._tdcss('45%').'>'.$filename.'</td>'."\n";
			echo '<td'._tdcss('20%','center').'>'._convertsize($filesize).'</td>';
			
			echo '<td'._tdcss('10%','center').'><span data-id="'.$i.'" data-did="'.$row['did'].'" data-filename="'.$filename.'" data-filesize="'.$filesize.'" 
			data-atitle'.$i.'="" class="dm-sprite attachment" alt="Attach File to Download" title="Attach File to Download"></span></td>';
	...
	if($db->sql_numrows($result) > 0)
	{
		while($file = $db->sql_fetchrow($result))
		{
			...
			echo '<td'._tdcss('45%',FALSE,FALSE,FALSE,TRUE).'>'.$filename.((!file_exists(_FILE_REPOSITORY_DIR.$file['filename'])) ? '<span 
			style="color: darkred;">&nbsp;'.$lang_new[$module_name]['FILE_MISSING'].'</span>' : '').'</td>'."\n";
	...
	if($db->sql_numrows($result) > 0)
	{
		while($screen = $db->sql_fetchrow($result))
		{
			...
			echo '<td'._tdcss('45%',FALSE,FALSE,FALSE,TRUE).'><a'._ls().get_image_viewer('screens').' href="'._FILE_REPOSITORY_SCREENS.$screen['filename'].'" 
			title="'.$screen['title'].'">'.$screen['filename'].'</a>'.((!file_exists(_FILE_REPOSITORY_SCREENS.$screen['filename'])) ? '<span 
			style="color: darkred;">&nbsp;'.$lang_new[$module_name]['FILE_MISSING'].'</span>' : '').'</td>'."\n";
...
function _file_repository_upload_screens()
{
	...
	if(move_uploaded_file($_FILES['supload']['tmp_name'], NUKE_BASE_DIR.$_POST['screendir'].$newfilename))
	{
		...
		$db->sql_query("INSERT INTO `"._FILE_REPOSITORY_SCREENSHOTS."` (`pid`, 
		                                                                `did`, 
																	 `active`, 
																   `filename`, 
																       `size`, 
																  `submitter`, 
																      `title`) VALUES (NULL, '".$_POST['did']."', 1, '".$newfilename."', '".$size."', '".$userinfo['username']."', '".$_POST['title']."')");
		$pid = $db->sql_nextid();
        // Return a JSON array with all the information
...
function _file_repository_client_uploads()
{
	...
	if ( $num_client_uploads > 0 ):
		...
		while($submitted = $db->sql_fetchrow($result)):
			...
			echo '<td'._tdcss('30%',FALSE,_sc()).'><a'._ls().' href="modules.php?name=Profile&amp;mode=viewprofile&amp;u='.$submitted['suid'].'" 
			target="_blank">'.$submitted['sname'].'</a>&nbsp;(<a'._ls().' href="mailto:'.$submitted['semail'].'">'.$submitted['semail'].'</a>)</td>'."\n";
			
			echo '<td'._tdcss('20%','center',_sc()).'>'._timestamp($submitted['date'],$settings['date_format']).'</td>'."\n";
			
			echo '<td'._tdcss('5%','center',_sc()).'><a href="'.$admin_file.'.php?op='._MODNAME.'&amp;action=deleteitem&did='.$submitted['did'].'&amp;area=clientuploads"><span 
			class="dm-sprite delete-button"></span></a></td>'."\n";
