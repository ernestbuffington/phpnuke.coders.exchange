...
function _file_repository_configuration()
{
	...
	$colorbox = ((file_exists('includes/jquery/jquery.colorbox.js') 
	|| file_exists('includes/js/colorbox/jquery.colorbox-min.js') 
	|| file_exists('includes/js/jquery.colorbox.php')) ? array('colorbox' => $lang_new[$module_name]['CB']) : array());
	
	$fancybox = ((file_exists('includes/jquery/jquery.fancybox.js') 
	|| file_exists('includes/fancybox/jquery.fancybox.js') 
	|| file_exists('includes/js/jquery.fancybox.js') 
	|| file_exists('includes/js/jquery.fancybox.php')) ? array('fancybox' => $lang_new[$module_name]['FB']) : array());
    ...
    //	THEME SETTINGS
	if(!defined('NUKE_FILE') && $settings['download_view'] == 1)
	{
		$alternateWidth = '20%';
		$colspan 		= false;
	}
	elseif(!defined('NUKE_FILE') && $settings['download_view'] == 0)
	{
		$alternateWidth = false;
		$colspan 		= 2;
	}
	elseif(defined('NUKE_FILE') && $settings['download_view'] == 1)
	{
		$alternateWidth = '25%';
		$colspan 		= false;
	}
