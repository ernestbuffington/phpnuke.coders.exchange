	...
	if ($BannedShouter != "yes") 
	{
        if ($ShoutSubmit == "ShoutPost") 
		{
			...
			if (!$ShoutError) {
				...
				if ($is_user) {
					$day = FormatDate('d/m/Y', time(), $userinfo['nuke_user_timezone']);
					$time = FormatDate('H:i', time(), $userinfo['nuke_user_timezone']);
				} else {
        ...
        */
        ...
        if ($stickyRow0) {
            ...
            if ($conf['date']== 'yes') {
                if ($is_user) {
                    $unixTime = FormatDate($userinfo['nuke_user_dateformat'], $stickyRow0['timestamp'], $userinfo['nuke_user_timezone']);
        ...
        if ($stickyRow1) {
            ...
            if ($conf['date']== 'yes') {
                if ($is_user) {
                    $unixTime = FormatDate($userinfo['nuke_user_dateformat'], $stickyRow1['timestamp'], $userinfo['nuke_user_timezone']);
                    $mid_content .= "<br />$unixTime";
                } else {
     ...
     ******************************************************/
            ...
            if ($conf['date']== 'yes') 
			{
                if (!empty($row['timestamp'])) 
				{
                    ...
                    if ($is_user) 
					{
                        $unixTime = FormatDate($userinfo['nuke_user_dateformat'], $row['timestamp'], $userinfo['nuke_user_timezone']);
