			      ...
			      &$online_status)
{
    ...
    $search_img = '<a href="'.$temp_url.'"><img src="'.$images['icon_search'].'" alt="'.sprintf($lang['Search_nuke_user_posts'], $row['username']).'" 
	title="'.sprintf($lang['Search_nuke_user_posts'], $row['username']).'" border="0" /></a>';
    $search = '<a href="'.$temp_url.'">'.sprintf($lang['Search_nuke_user_posts'], $row['username']).'</a>';
...
{
    if(!is_user()) 
    redirect(append_sid("login.$phpEx?redirect=groupcp.$phpEx&" . POST_GROUPS_URL . "=$nuke_group_id", true));
    
    $sql = "SELECT group_moderator FROM ".GROUPS_TABLE." WHERE nuke_group_id = '$nuke_group_id'";
    ...
    $message = $lang['Not_group_moderator'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id")
	.'">', '</a>').'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
    endif;
    
    $sql = "UPDATE ".GROUPS_TABLE." SET group_type = ".intval($_POST['group_type'])." WHERE nuke_group_id = '$nuke_group_id'";
	if(!($result = $db->sql_query($sql))) 
    message_die(GENERAL_ERROR, 'Could not obtain user and group information', '', __LINE__, __FILE__, $sql);
    ...
    $message = $lang['Group_type_updated'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id").'">', '</a>')
	.'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
    message_die(GENERAL_MESSAGE, $message);
} 
elseif(isset($_POST['joingroup']) && $nuke_group_id) 
{
  ...
  OR (g.group_count <= '".$userdata['nuke_user_posts']."' 
  AND g.group_count_max > '".$userdata['nuke_user_posts']."')) 
  ...
  message_die(GENERAL_ERROR, 'Could not obtain user and group information', '', __LINE__, __FILE__, $sql);
    ...
    if($row = $db->sql_fetchrow($result)): 
      $is_autogroup_enable = ($row['group_count'] <= $userdata['nuke_user_posts'] && $row['group_count_max'] > $userdata['nuke_user_posts']) ? true : false;
    ...
    # Mod: Auto Group v1.2.2 END
            do 
			{
                if ($userdata['user_id'] == $row['user_id']): 
					...
					.POST_GROUPS_URL."=$nuke_group_id").'">', '</a>').'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
            ...
            $message = $lang['This_closed_group'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id").'">', '</a>')
			.'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
            message_die(GENERAL_MESSAGE, $message);
       endif;
    ...
    $sql = "INSERT INTO ".USER_GROUP_TABLE." (nuke_group_id, user_id, user_pending) VALUES ('$nuke_group_id', ".$userdata['user_id'].",'".(($is_autogroup_enable) ? 0 : 1)."')";
    
	if (!($result = $db->sql_query($sql))) 
    message_die(GENERAL_ERROR, "Error inserting user group subscription", "", __LINE__, __FILE__, $sql);
    
    $sql = "SELECT u.user_email, u.username, u.nuke_user_lang, g.group_name FROM (".USERS_TABLE." u, ".GROUPS_TABLE." g) WHERE u.user_id = g.group_moderator AND g.nuke_group_id = '$nuke_group_id'";
    ...
    # Mod: Auto Group v1.2.2 END
	  ...
	  $content = str_replace('{U_GROUPCP}', '<a href="'.$server_url.'&'.POST_GROUPS_URL.'='.$nuke_group_id.'&validate=true">'.$server_url.'&'.POST_GROUPS_URL.'='
	  .$nuke_group_id.'&validate=true</a>', $content );
...
elseif(isset($_POST['unsub']) || isset($_POST['unsubpending']) && $nuke_group_id) 
{
    ...
    redirect('modules.php?name=Your_Account&amp;redirect=groupcp.php&amp;'.POST_GROUPS_URL.'='.$nuke_group_id);
	elseif ($sid !== $userdata['session_id']) 
    message_die(GENERAL_ERROR, $lang['Session_invalid']);
    ...
    if($confirm): 
        ...
        $sql = "UPDATE ".USERS_TABLE." SET user_color_gc = '', user_color_gi  = '', nuke_user_rank = 0 WHERE user_id = ".$userdata['user_id'];
        ...
        $sql = "DELETE FROM ".USER_GROUP_TABLE." WHERE user_id = ".$userdata['user_id']." AND nuke_group_id = '$nuke_group_id'";
        ...
        if($userdata['user_level'] != ADMIN && $userdata['user_level'] == MOD): 
		    ...
		    AND aa.nuke_group_id = ug.nuke_group_id AND aa.auth_mod = '1'";
        ...
        $message = $lang['Unsub_success'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id") 
		.'">', '</a>').'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
        message_die(GENERAL_MESSAGE, $message);
...
elseif($nuke_group_id) 
{
    ...
    $sql = "SELECT g.group_moderator, 
			...
			ON aa.nuke_group_id = g.nuke_group_id) 
			WHERE g.nuke_group_id = '$nuke_group_id'";
        ...
        if(!empty($_POST['add']) || !empty($_POST['remove']) || isset($_POST['approve']) || isset($_POST['deny'])): 
            ...
            if(isset($_POST['add'])): 
                ...
                if(!($row = $db->sql_fetchrow($result))): 
				  ...
				  ."=$nuke_group_id")."\">", "</a>")."<br /><br />".sprintf($lang['Click_return_index'], "<a href=\"".append_sid("index.$phpEx")."\">", "</a>");
                ...
                if($row['user_id'] == ANONYMOUS): 
                    ...
                    $message = $lang['Could_not_anon_user'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id") 
					.'">', '</a>').'<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');                    
                    message_die(GENERAL_MESSAGE, $message);
                endif;
                ...
                if(!($db->sql_fetchrow($result))): 
                    $sql = "INSERT INTO ".USER_GROUP_TABLE." (user_id, nuke_group_id, user_pending) VALUES (".$row['user_id'].", '$nuke_group_id', '0')";
                    if(!$db->sql_query($sql)) 
                    ...
                    $group_sql = "SELECT group_name FROM ".GROUPS_TABLE." WHERE nuke_group_id = '$nuke_group_id'";
                    ...
                    # Mod: Group Colors and Ranks v1.0.0 START
                    add_group_attributes($row['user_id'], $nuke_group_id);
                    # Mod: Group Colors and Ranks v1.0.0 END
					...
					$content = str_replace('{U_GROUPCP}', '<a href="'.$server_url.'&'.POST_GROUPS_URL.'='.$nuke_group_id.'">'.$server_url.'&'.POST_GROUPS_URL.'='.
					$nuke_group_id.'</a>', $content );
                    ...
                    $message = $lang['User_is_member_group'].'<br /><br />'.sprintf($lang['Click_return_group'], '<a href="'.append_sid("groupcp.$phpEx?".POST_GROUPS_URL."=$nuke_group_id") 
					.'">', '</a>'). '<br /><br />'.sprintf($lang['Click_return_index'], '<a href="'.append_sid("index.$phpEx").'">', '</a>');
                 ...
                 if(((isset($_POST['approve']) || isset($_POST['deny'])) && isset($_POST['pending_members'])) || (isset($_POST['remove']) && isset($_POST['members']))): 
                    ...
                    if(isset($_POST['approve'])): 
                        ...
                        # Mod: Group Colors and Ranks v1.0.0 START
                        global $prefix;
                        $sql_color = "SELECT group_color FROM " . GROUPS_TABLE . " WHERE nuke_group_id = '$nuke_group_id'";
						...
						if($color): 
                            $sql_color = "SELECT group_color, nuke_group_id FROM ".$prefix."_bbadvanced_username_color WHERE nuke_group_id = '$color'";
                            if (!$result_color = $db->sql_query($sql_color)) 
                                message_die(GENERAL_ERROR, 'Could not gather group color', '', __LINE__, __FILE__, $sql);
                            $row_color = $db->sql_fetchrow($result_color);
                            $db->sql_freeresult($result_color);
                        ...
                        $sql_rank = "SELECT group_rank FROM ".GROUPS_TABLE." WHERE nuke_group_id = '$nuke_group_id'";
                        ...
                        $sql = "nuke_user_rank = '".$row_rank['group_rank']."'";
                        elseif ($row_color["group_color"] && !$row_rank['group_rank']) 
                        $sql = "user_color_gc = '".$row_color["group_color"]."', user_color_gi  = '--".$row_color["nuke_group_id"]."--'";
                        elseif ($row_color['group_color'] && $row_rank['group_rank']) 
                        $sql = "nuke_user_rank = '".$row_rank['group_rank']."', user_color_gc = '".$row_color["group_color"]."', user_color_gi  = '--".$row_color["nuke_group_id"]."--'";
                        else 
                        $sql = "";
                        ...
                        $sql = "UPDATE ".USER_GROUP_TABLE." SET user_pending = 0 WHERE user_id IN ($sql_in) AND nuke_group_id = '$nuke_group_id'";
                 ...
                 elseif(isset($_POST['deny']) || isset($_POST['remove'])): 
                        ...
                        # Mod: Group Colors and Ranks v1.0.0 START
                        $sql = "UPDATE ".USERS_TABLE." SET user_color_gc = '', user_color_gi  = '', nuke_user_rank = 0 WHERE user_id IN ($sql_in)";
                        if (!$db->sql_query($sql)) 
                        message_die(GENERAL_ERROR, 'Could not remove color from user', '', __LINE__, __FILE__, $sql);
                        # Base: Caching System v3.0.0 START
                        $cache->delete('UserColors', 'config');
                    ...
                    if(isset($_POST['approve'])): 
						...
						$content = str_replace('{U_GROUPCP}', '<a href="'.$server_url.'&'.POST_GROUPS_URL.'='.$nuke_group_id.'">'.$server_url.'&'.
						POST_GROUPS_URL.'='.$nuke_group_id.'</a>', $content );
    ...
    # Get group details
    $sql = "SELECT * FROM ".GROUPS_TABLE." WHERE nuke_group_id = '$nuke_group_id' AND group_single_user = '0'";
    if(!($result = $db->sql_query($sql))) 
    ...
    # Mod: Auto Group v1.2.2 START
    $is_autogroup_enable = ($group_info['group_count'] <= $userdata['nuke_user_posts'] && $group_info['group_count_max'] > $userdata['nuke_user_posts']) ? true : false;
    # Mod: Auto Group v1.2.2 END
    ...
    list($the_group_name) = $db->sql_ufetchrow("SELECT `group_name` FROM ".GROUPS_TABLE." WHERE `nuke_group_id`=$nuke_group_id", SQL_NUM);

    $template->assign_vars(array(
	'GROUPS_LINK' => '<a href="modules.php?name=Groups">Member Groups</a> <i class="fas fa-arrow-right"></i> ',
	'GROUPS_LIST_INFO_LINK' => '<a href="modules.php?name=Groups&amp;g='.$nuke_group_id.'">'.$the_group_name.'</a>',
    ...
    switch($group_moderator['nuke_user_avatar_type'])
    {
      case USER_AVATAR_UPLOAD:
    ...
    # Mod: Forum Index Avatar Mod v3.0.0 START
    switch(isset($group_members[$i]['nuke_user_avatar_type']))
    {
      case USER_AVATAR_UPLOAD:
    ...
    if($is_moderator) 
	{
        ...
        if($modgroup_pending_count) 
		{
            for($i = 0; $i < $modgroup_pending_count; $i++) 
			{
              ...
              # Mod: Forum Index Avatar Mod v3.0.0 START
              switch($modgroup_pending_list[$i]['nuke_user_avatar_type'])
              {
                  case USER_AVATAR_UPLOAD:
