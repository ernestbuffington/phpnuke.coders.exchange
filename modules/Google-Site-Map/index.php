...
while ($row2 = $db->sql_fetchrow($result2)): 
    ...
    # dont do anything
     ...
     # Select all groups
	 global $prefix;
     $sql = $db->sql_query("select nuke_group_id, group_name from ".$prefix."_bbgroups where group_description <> 'Personal User'");
     while(list($nuke_group_id, $group_name) = $db->sql_fetchrow($sql)): 
       if (is_user())
       print '<td></td><td><i style="vertical-align: middle;" 
	   class="fa fa-lock green"></i>&nbsp;<a href="modules.php?name=Groups&amp;g='.$nuke_group_id.'">Users Group &#187; '.$group_name.' &#187; Join Today</a></td></tr>'; 
	   else
       print '<td></td><td><span class="brightred"><i style="vertical-align: middle;" 
	   class="fa fa-lock"></i></span>&nbsp;<a href="modules.php?name=Groups&amp;g='.$nuke_group_id.'">Users Group &#187; '.$group_name.'</a><a  
	   href="modules.php?name=Your_Account&op=new_user"> &#187; Login or Create New User Account</a></td></tr>'; 
	   $sitemap->addItem('/modules.php?name=modules.php?name=Groups&g='.$nuke_group_id.'', '0.8', 'monthly', 'Jun 25');
     endwhile;
	...
	switch($link): 
		...
		# Downloads 
		case 'Downloads':
			$result3 = $db->sql_query("SELECT `cid`, `title` FROM `".$prefix."_downloads_categories` WHERE `cid`>1 AND `parentid`=0 ORDER BY `title`");
			while ($row3 = $db->sql_fetchrow($result3)): 
			    ...
			    $result4 = $db->sql_query('SELECT `cid`, `title` FROM `'.$prefix.'_downloads_categories` WHERE `cid`>1 AND `parentid`="'.$cid1.'" ORDER BY `title`');
