...
//-------------------------------------------------------------------------
		$response = array(
			...
			'uploaded'		=> formatTimestamp_to_date('D M d, Y g:i a', time(), $userinfo['nuke_user_timezone']), 
...
//-------------------------------------------------------------------------	
	...
	while($uploadinfo = $db->sql_fetchrow($result))
	{
		...
		echo '    <td'.tablecss('20%','center','row1').'>'._timestamp('M d, Y | G:i', $uploadinfo['uploaded'], $userinfo['nuke_user_timezone']).'</td>';
