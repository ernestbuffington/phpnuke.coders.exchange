...
if (is_mod_admin()) {
    ...
    function newsletter_selection($fieldname, $current) {
        ...
        if (!isset($groups)) {
            ...
            $groupsResult = $db->sql_query("SELECT nuke_group_id, group_name FROM ".$prefix."_bbgroups WHERE group_single_user=0");
    ...
    if (isset($_POST['discard'])) {
        ...
        if ($n_group == 0) {
            ...
            $n_group -= 2;
            $query = "SELECT u.username, u.user_email FROM ".$user_prefix."_users u, ".$prefix."_bbuser_group g WHERE u.user_level>0 AND g.nuke_group_id=$n_group AND u.user_id = g.user_id AND user_pending=0";
        } else {
    ...
    if (isset($_POST['preview'])) {
        ...
        if ($group == 0) {
            ...
            $nuke_group_id = $group-2;
            list($num_users) = $db->sql_fetchrow($db->sql_query('SELECT COUNT(*) FROM '.$prefix.'_bbuser_group WHERE nuke_group_id="'.$nuke_group_id.'" AND user_pending="0"'));
            list($group_name) = $db->sql_ufetchrow("SELECT group_name FROM ".$prefix."_bbgroups WHERE nuke_group_id=$nuke_group_id", SQL_NUM);
        } else {
