...
/************************************************************************
   ...
   Theme Management
   $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
   Copyright (c) 2023 by The PHP-Nuke Titanium Group

   Filename      : themes.php
   Author(s)     : Ernest Buffington (TheGhost), JeFFb68CAM
   Version       : 2.0.0
...
/*****[CHANGES]**********************************************************
-=[Base]=-
      PHP Patched                             v8.2.4           04/07/2023
-=[Mods]=-	  
      Theme Backup and Download               v1.0.0           04/07/2023
	  Groups Themes Now Works                 v1.0.0           04/07/2023
	  Removed Zip Install                                      04/07/2023
 ************************************************************************/
...
{
	DisplayError('<span style="font-size: 13px; font-weight: bold;">ERROR</span><br /><br />'.$error_message.'<br /><br />'._GOBACK, true);
}
...
function downloadTheme($theme)
{
    ...
    function ZipTheme($source, $destination)
    {
	  ...
	  if (is_god($admin))
	  {
         ...
         if (!$ZipTheme->open($destination, ZIPARCHIVE::CREATE)) {
            return false;
        }
        ...
        if (is_dir($source) === true)
        {
            ...
            foreach ($files as $file)
            {
                ...
                if (is_dir($file) === true)
                {
                   $ZipTheme->addEmptyDir(str_replace($source . '/', '', $file . '/'));
                }
                else if (is_file($file) === true)
                {
                    $ZipTheme->addFromString(str_replace($source . '/', '', $file), file_get_contents($file));
                }
            }
        }
        else if (is_file($source) === true)
        {
            $ZipTheme->addFromString(basename($source), file_get_contents($source));
        }

        return $ZipTheme->close();
	  }
    }
	...
	if (is_god($admin))
	{
		...
		ZipTheme($themedir, NUKE_THEMES_SAVE_DIR.$theme.'-'.$random.'.zip');
...
function display_main()
{
    ...
    function make_a_row($theme)
	{
        ...
        $download_link  = (!theme_exists($theme['theme_name'])) 
		? get_evo_icon('evo-sprite folder-archive', _THEMES_DOWNLOAD) : "<a href=\"$admin_file.php?op=downloadTheme&amp;theme=" . $theme['theme_name'] . "\">".get_evo_icon('evo-sprite folder-archive', _THEMES_DOWNLOAD)."</a>";
        ...
        echo "" . $default_link . " \n";
        echo "" . $activate_link . " \n";
        echo "<a data-colorbox href=\"index.php?tpreview=" . $theme['theme_name'] . "\" target=\"_blank\">".get_evo_icon('evo-sprite search', _THEMES_VIEW)."</a> \n";
        echo "" . $download_link . " \n";
	...
	if (is_array($uninstalled_themes))
	{
		foreach($uninstalled_themes as $theme)
		{
			...
			echo "<a class=\"theme-preview\" href=\"index.php?tpreview=" . $theme . "\" target=\"_blank\">" . _THEMES_VIEW . "</a> ]\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
	}
	...
	echo "<table border='0' align='center' width='99%' cellpadding='4' cellspacing='1' class='forumline'>\n";
	echo "<tr>\n";
	echo "<th width='100%' align='center'><span class=\"title\" style=\"font-weight: bold\">Theme Management v2.6.3</i></span></th>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	echo "<td width='100%' class='row1' align='center'>\n";
	echo 'Last Update: 04/09/2023 1:35 AM';
	echo "</td>\n";
	echo "</tr>\n";
	echo "</table>\n";
	echo "</div>"; 
...
function theme_edit($theme_name)
{
    ...
    $groupsResult = $db->sql_query("SELECT nuke_group_id, group_name FROM ".$prefix."_bbgroups WHERE group_description <> 'Personal User'");
	...
	{
        echo "<tr>\n";
		echo "<td align='center' colspan='2'><span style='color:green'>" . _THEMES_ADV_COMP . "</span></td>\n";
		echo "</tr>\n";
	...
	echo "</fieldset>\n";
    echo "<br />\n";
    echo "<div align=\"center\"><input type='submit' class='mainoption' value='".$admlang['global']['save_changes']."'/><input type='button' class='mainoption' value='Go Back' onclick='javascript:history.go(-1)' /></div>\n";
    echo "</td>\n";
    echo "</tr>";
...
function theme_install($theme_name)
{
	...
	if (!isset($theme_info['groups'])) { $theme_info['groups'] = '-'; }
...
function update_theme($post)
{
	...
	if(!isset($post['groups'])) { $post['groups'] = '';	}
	...
	else 
	{
		...
		echo "<div align='center'>\n"; 
		echo "<span style=\"font-weight: bold\"><span style='color:red'>" . _THEMES_UPDATEFAILED . "</span></span><br /><br />\n";
		echo "<a href=\"$admin_file.php?op=themes\">" . _THEMES_RETURN . "</a>\n";
		echo "</div>\n"; 
...
function install_save($post){
	...
	if(isset($_GET['custom_theme'])): 
	  if(!isset($theme_name)):
	  $theme_name = $_GET['custom_theme'];
	  endif;
    endif;
	
	if(!isset($_GET['custom_theme'])):
	  $post['custom_name'] = $post['theme_name'];
	endif; 
    ...
    if (file_exists(NUKE_THEMES_DIR.$post['theme_name'].'/theme_info.php'))
	{
       ...
       $sql = "REPLACE INTO " . $prefix . "_themes VALUES('" . $post['theme_name'] . "', 
	                                                          '" . $post['groups'] . "', 
													     '" . $post['permissions'] . "', 
													     '" . $post['custom_name'] . "', 
													          '" . $post['active'] . "', 
	...
	else 
	{
	   ...
	   $theme_info = '93%:::#000000:::#151515:::#262626:::#262626:::#ccc:::#ccc:::Go to Theme Options to Edit Footer Message Line 1:::Go to Theme Options to Edit Footer Message Line 2:::green:::dark';
	   
	   if(!isset($post['groups'])) { $post['groups'] = '-'; }
	   $sql2 = "REPLACE INTO " . $prefix . "_themes VALUES('" . $post['theme_name'] . "', 
	                                                          '" . $post['groups'] . "', 
													     '" . $post['permissions'] . "', 
													     '" . $post['custom_name'] . "', 
													          '" . $post['active'] . "', 
