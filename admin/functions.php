...
function track_phpnuke_current_version() 
{
	global $admin_file, $admlang;
	/**
	...
	if($version_check_cache['version'] == NUKE_VERSION):
		...
		$new_version_number = NUKE_VERSION;
	...
	$return .= '  <tr>'."\n";
	$return .= '    <td style="height:15px; font-size: 13px; width:65%;">PHP-Nuke '.$new_version_number.'&nbsp;&nbsp;<font size="1">'.$version_desc.'</font></td>'."\n";
...
function GraphicAdmin($pos=1)
{
    ...
    */
	...
	# check evo version
	echo track_phpnuke_current_version();
...
function track_phpnuke_current_version_bs() 
{
	global $admin_file, $admlang;
	/**
	 * Version checker json
...
function administration_panel( $pos = 1 )
{
	...
	$versioncheck = track_phpnuke_current_version_bs();
			...
			<div class="col-sm-6 col-md-6 col-lg-6 col-xl-3 admin-info-cubes">
				<a href="<?php echo $versioncheck['update_url']; ?>">
					<div class="small-box <?php echo $versioncheck['class'] ?>">
						<h2 class="title"><?php echo $versioncheck['new_version_number'].' '.NUKE_EDITION ?></h2>
