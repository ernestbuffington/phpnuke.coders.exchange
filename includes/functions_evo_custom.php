...
function get_user_avatar($user_id) {
	...
	if ( $user_id == $userinfo['user_id'] ) {
		 ...
		 $nuke_user_avatar_type  = $userinfo['nuke_user_avatar_type'];
		 $user_avatar_allow = $userinfo['user_allowavatar'];
		 $user_avatar_show  = $userinfo['user_showavatars'];
	} else {
		list($user_avatar, $nuke_user_avatar_type, $user_avatar_allow, $user_avatar_show) = $db->sql_ufetchrow("SELECT user_avatar, nuke_user_avatar_type, user_allowavatar, user_showavatars FROM ".USERS_TABLE." WHERE user_id = '" . $user_id . "' LIMIT 1");
	}
	$poster_avatar = '';
	if ( $nuke_user_avatar_type && $user_id != ANONYMOUS && $user_avatar_allow && $user_avatar_show && !empty($user_avatar)) {
