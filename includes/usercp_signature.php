...
if ($submit)
{
    ...
    if ( isset($signature) )
    {
        ...
        SET nuke_user_sig = '" . str_replace("\'", "''", $signature) . "', nuke_user_sig_bbcode_uid = '$bbcode_uid'
...
else if ($mode)
{
    ...
    $signature = ( $signature_bbcode_uid != '' ) ? preg_replace("/:(([a-z0-9]+:)?)$signature_bbcode_uid\]/si", ']', $userdata['nuke_user_sig']) : $userdata['nuke_user_sig'];
    $bbcode_uid = $userdata['nuke_user_sig_bbcode_uid'];
    $nuke_user_sig = prepare_message($userdata['nuke_user_sig'], $html_on, $bbcode_on, $smilies_on, $bbcode_uid);
