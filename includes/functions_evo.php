...
function get_user_field($field_name, $user, $is_name = false) 
{
	...
	if (!isset($actual_user[$user])) 
	{
        ...
        $result = $db->sql_query('SELECT g.nuke_group_id, 
		                               g.group_name, 
								g.group_single_user 
								  
								  FROM ('.GROUPS_TABLE.' AS g 
								  INNER JOIN '.USER_GROUP_TABLE.' 
								  AS ug ON (ug.nuke_group_id=g.nuke_group_id AND ug.user_id="'.$actual_user[$user]['user_id'].'" 
...
function GetColorGroups($in_admin = false) 
{
	...
	{
        $ColorGroupsCache = '';
        $result = $db->sql_query("SELECT `nuke_group_id`, `group_name`, `group_color`, `group_weight` FROM `".AUC_TABLE."` WHERE `nuke_group_id`>'0' ORDER BY `group_weight` ASC");
        $back = ($in_admin) ? '&amp;menu=1' : '';
    
	    while (list($nuke_group_id, $group_name, $group_color, $group_weight) = $db->sql_fetchrow($result)) 
		{
            $ColorGroupsCache .= '&nbsp;[&nbsp;<strong><a href="'. append_sid('auc_listing.php?id='. $nuke_group_id.$back) .'"><span class="genmed" style="color:#'. $group_color .';">'. $group_name .'</span></a></strong>&nbsp;]&nbsp;';
        }
        $db->sql_freeresult($result);
...
function group_selectbox($fieldname, $current=0, $mvanon=false, $all=true) 
{
    ...
    if (!isset($groups)):
        ...
        $result = $db->sql_query('SELECT `nuke_group_id`, `group_name` FROM `'.GROUPS_TABLE.'` WHERE `group_single_user` = 0', true);
...
function GetRank($user_id) 
{
    ...
    list($nuke_user_rank, $nuke_user_posts) = $db->sql_ufetchrow("SELECT nuke_user_rank, nuke_user_posts FROM " . $user_prefix . "_users WHERE user_id = '" . $user_id . "'", SQL_NUM);
...
function add_group_attributes($user_id, $nuke_group_id) 
{
    global $prefix, $db, $board_config, $cache;

    if ($user_id <= 2) return true;

    $sql_color = "SELECT `group_color` FROM `" . $prefix . "_bbgroups` WHERE `nuke_group_id` = '$nuke_group_id'";
	...
	$sql_rank = "SELECT `group_rank` FROM `" . $prefix . "_bbgroups` WHERE `nuke_group_id` = '$nuke_group_id'";
	...
	elseif(isset($row_color['group_color']) && !isset($row_rank['group_rank'])) 
	{
        $sql = "`user_color_gc` = '".$row_color['group_color']."',
              `user_color_gi`  = '--".$row_color['nuke_group_id']."--'";
    } 
	elseif (isset($row_color['group_color']) && isset($row_rank['group_rank'])) {
        $sql = "`nuke_user_rank` = '".$row_rank['group_rank']."',
...
function remove_group_attributes($user_id, $nuke_group_id) 
{
    global $prefix, $db, $board_config, $cache;
    if (empty($user_id) && !empty($nuke_group_id) && $nuke_group_id != 0) {
        $sql = "SELECT `user_id` FROM `".$prefix."_bbuser_group` WHERE `nuke_group_id`=".$nuke_group_id;
