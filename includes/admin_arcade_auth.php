...
$nuke_group_id = get_var_gf(['name' => POST_GROUPS_URL, 'intval' => true, 'default' => 0]);
...
if (isset($_POST['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $nuke_group_id ) )) {
    ...
    if ( $mode == 'user' )
    {
        ...
        $nuke_group_id = $row['nuke_group_id'];
        $db->sql_freeresult($result);
    }
    $sql = "SELECT arcade_catid FROM " . AUTH_ARCADE_ACCESS_TABLE . " WHERE nuke_group_id = $nuke_group_id";
    if ( !($result = $db->sql_query($sql)) )
    {
        message_die(GENERAL_ERROR, 'Could not select info from user/user_group table', '', __LINE__, __FILE__, $sql);
    }
    ...
    {
        $sql = "INSERT INTO " . AUTH_ARCADE_ACCESS_TABLE . " ( nuke_group_id, arcade_catid) VALUES " . $liste_a_creer;
        if ( !$db->sql_query($sql) )
        {
    ...
    $db->sql_freeresult($result);
    $sql = "SELECT u.user_id, u.username, u.user_level, g.nuke_group_id, g.group_name, g.group_single_user FROM " . USERS_TABLE . " u, " . GROUPS_TABLE . " g, " . USER_GROUP_TABLE . " ug WHERE ";
    ...
    $db->sql_freeresult($result);
    $sql = ( $mode == 'user' ) ? "SELECT aa.arcade_catid FROM " . AUTH_ARCADE_ACCESS_TABLE . " aa, " . USER_GROUP_TABLE . " ug, " . GROUPS_TABLE. " g WHERE ug.user_id = $user_id AND g.nuke_group_id = ug.nuke_group_id AND aa.nuke_group_id = ug.nuke_group_id AND g.group_single_user = 1" : "SELECT arcade_catid FROM " . AUTH_ARCADE_ACCESS_TABLE . " WHERE nuke_group_id = $nuke_group_id";
    if ( !($result = $db->sql_query($sql)) )
    {
	...
	foreach ($ug_info as $i => $singleUg_info) 
	{
		...
		$singleUg_info['nuke_group_id'] = $singleUg_info['nuke_group_id'] ?? '';
