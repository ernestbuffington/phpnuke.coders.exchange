...
{
	$user_fields_array = ['nuke_user_rank', 'user_rank2', 'user_rank3', 'user_rank4', 'user_rank5'];
	$nuke_user_ranks_array = ['rank_01', 'rank_01_img', 'rank_02', 'rank_02_img', 'rank_03', 'rank_03_img', 'rank_04', 'rank_04_img', 'rank_05', 'rank_05_img'];
	$nuke_user_ranks = [];
	...
	foreach (array_keys($nuke_user_ranks_array) as $j) {
	...
	for($j = 0; $j < $itemsCount; $j++):
		...
		elseif($is_banned == true):
			...
			$day_diff = (int) ((time() - (int) $user_row['nuke_user_regdate']) / 86400);
       ...
       switch($ranks_sql['ranksrow'][$j]['rank_special']):
   					case '1':
   						...
   						break;
   					case '0':
   						if(($user_row[$singleUser_fields_array] == '0') && ($user_row['nuke_user_posts'] >= $ranks_sql['ranksrow'][$j]['rank_min']))
   						$rank_sw = true;
   						break;
   					case '-1':
