...
$mode == 'register' ):
	...
	$nuke_user_style = (isset($_POST['style'])) ? $_POST['style'] : '';

	if(!empty($_POST['language'])):
		if(preg_match('/^[a-z_]+$/i',$_POST['language'])):
		$nuke_user_lang = htmlspecialchars($_POST['language']);
		else:
		$error = true;
	...
	$nuke_user_dateformat = (!empty($_POST['dateformat'])) ? trim(htmlspecialchars($_POST['dateformat'])) : $board_config['default_dateformat'];
	...
	$nuke_user_avatar_type = (empty($user_avatar_local) && $mode == 'editprofile') ? $userdata['nuke_user_avatar_type'] : '0';
...
if(isset($_POST['submit'])):
	...
	elseif((!empty($user_avatar_upload) || !empty($user_avatar_name) ) && $board_config['allow_avatar_upload'])
	{
		if(!empty($user_avatar_upload)):
			...
			$avatar_sql = user_avatar_upload($mode, 
					 ...
					 $userdata['nuke_user_avatar_type'], 
									        $error, 
										$error_msg, 
							   $user_avatar_upload, 
							     $user_avatar_name, 
	...
	if(!$error)
	{
		...
		if($mode == 'editprofile')
		{
			...
			= '$notifyreply', nuke_user_notify_pm = '$notifypm', user_allow_mass_pm = '$allow_mass_pm', user_popup_pm 
			= '$popup_pm', nuke_user_timezone = '$nuke_user_timezone', user_time_mode = $time_mode, user_dst_time_lag = $dst_time_lag, nuke_user_dateformat 
			= '".str_replace("\'", "''",$nuke_user_dateformat)."', user_show_quickreply = '$user_show_quickreply', sceditor_in_source 
			= '$sceditor', user_quickreply_mode = '$user_quickreply_mode', user_wordwrap = '" . str_replace("\'", "''", $user_wordwrap) 
			. "', user_open_quickreply = $user_open_quickreply, nuke_user_lang = '".str_replace("\'", "''", $nuke_user_lang)."', theme 
			= '$nuke_user_style', user_active = '$user_active', user_actkey = '".str_replace("\'", "''", $user_actkey)."'".$avatar_sql.", user_gender 
			...
			if(!($result = $db->sql_query($sql))):
				...
				global $userinfo;
				$userinfo['theme'] = $nuke_user_style;
				UpdateCookie();
			endif;
 ...
 ******************************************************/
	...
	$signature = ($signature_bbcode_uid != '') ? preg_replace("/:(([a-z0-9]+:)?)$signature_bbcode_uid(=|\])/si", '\\3', $userdata['nuke_user_sig']) : $userdata['nuke_user_sig'];
 ...
 ******************************************************/
	...
	$nuke_user_style = $userdata['theme'];
/*****[BEGIN]******************************************
 [ Mod:    Force Word Wrapping - Configurator v1.0.16 ]
 ******************************************************/
