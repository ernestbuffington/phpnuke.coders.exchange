...
if(isset($profiledata['user_id']) && !empty($profiledata['user_id'])): 
    $sql = "SELECT group_name 
	        FROM ".GROUPS_TABLE." 
			LEFT JOIN ".USER_GROUP_TABLE." on ".USER_GROUP_TABLE.".nuke_group_id=".GROUPS_TABLE.".nuke_group_id 
...
// bots have no regdate
if(!isset($profiledata['nuke_user_regdate']))
$profiledata['nuke_user_regdate'] = 'Jan 01, 2000';
...
if(empty($profiledata['user_avatar'])) {
  $profiledata['user_avatar'] = 'robot.png';
  $profiledata['nuke_user_avatar_type'] = 1;
}

if($profiledata['nuke_user_avatar_type'] && $profiledata['user_allowavatar']):
    switch( $profiledata['nuke_user_avatar_type']):
        case USER_AVATAR_UPLOAD:
            $avatar_img = ($board_config['allow_avatar_upload']) ? '<img class="rounded-corners-profile" style="max-width: 150px;" 
			src="'.$board_config['avatar_path']. '/'. $profiledata['user_avatar'] . '" alt="" border="0" />' : '';
            break;
        case USER_AVATAR_REMOTE:
            $avatar_img = '<img class="rounded-corners-profile" style="max-width: 150px;" s
        ...
        case USER_AVATAR_GALLERY:
			...
			150px;" src="' . $board_config['avatar_gallery_path'] . '/' 
 ...
 ******************************************************/
 if(!isset($profiledata['user_from_flag']))
 $profiledata['user_from_flag'] = 'usa.png';
 
$location = (!empty($profiledata['user_from_flag']) ) ? '<span class="countries '.str_replace('.png','',$profiledata['user_from_flag']).'"></span>&nbsp;' : 'lol&nbsp;';
$location .= ($profiledata['user_from']) ? $profiledata['user_from'] : 'The InterWebs';
/*****[END]********************************************
 [ Mod:     Member Country Flags               v2.0.7 ]
 ******************************************************/
...
$search_img = '<a href="' . $temp_url . '"><img src="' . $images['icon_search'] . '" alt="' . sprintf($lang['Search_nuke_user_posts'], $profiledata['username']) . '" title="' . sprintf($lang['Search_nuke_user_posts'], $profiledata['username']) . '" border="0" /></a>';
 ...
 ******************************************************/
$nuke_user_sig = '';
if(!empty($profiledata['nuke_user_sig'])) {
    ...
    $signature_bbcode_uid = $profiledata['nuke_user_sig_bbcode_uid'];
    $signature = ( $signature_bbcode_uid != '' ) ? preg_replace("/:(([a-z0-9]+:)?)$signature_bbcode_uid\]/si", ']', $profiledata['nuke_user_sig']) : $profiledata['nuke_user_sig'];
    $bbcode_uid = $profiledata['nuke_user_sig_bbcode_uid'];
    $nuke_user_sig = prepare_message($profiledata['nuke_user_sig'], $html_on, $bbcode_on, $smilies_on, $bbcode_uid);
    ...
    if( $nuke_user_sig != '' )
    {
        ...
        $template->assign_block_vars('nuke_user_sig', array());
...
/*****[BEGIN]******************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
    'L_SEARCH_USER_POSTS' => sprintf($lang['Search_nuke_user_posts'], UsernameColor($profiledata['username'])),
/*****[END]********************************************
 [ Mod:    Advanced Username Color             v1.0.5 ]
 ******************************************************/
...
foreach ($xd_meta as $code_name => $info) {
    ...
    if ($info['field_type'] == 'date')
  		{
  				$value = create_date($userdata['nuke_user_dateformat'], $value, $userdata['nuke_user_timezone']);
  		}
    /*****[ENDE]*******************************************
     [ Mod:    XData Date Conversion               v0.1.1 ]
     ******************************************************/
    ...
    if ( $info['allow_bbcode'] && $profiledata['nuke_user_sig_bbcode_uid'] != '')
    {
