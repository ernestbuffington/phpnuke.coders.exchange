 ...
 ******************************************************/
    if ($userdata['user_level'] == USER) {
    	$sql = "SELECT ug.user_id, g.nuke_group_id as g_id, u.nuke_user_posts, g.group_count, g.group_count_max FROM ".USERS_TABLE." u, " . GROUPS_TABLE . " g
    		LEFT JOIN ". USER_GROUP_TABLE." ug ON g.nuke_group_id=ug.nuke_group_id AND ug.user_id=$user_id
    	...
    	while ($group_data = $db->sql_fetchrow($result))
    	{
            ...
            $user_add = ($group_data['group_count'] == $group_data['nuke_user_posts'] && $user_id!=ANONYMOUS) ? TRUE : FALSE;
            $user_remove = ($group_data['group_count'] > $group_data['nuke_user_posts'] || $group_data['group_count_max'] < $group_data['nuke_user_posts']) ? TRUE : FALSE;
    		...
    		if ( $user_already_added && $user_remove)
    		{
    			...
    			$sql = "DELETE FROM " . USER_GROUP_TABLE . "
    				WHERE nuke_group_id=".$group_data['g_id']."
    				AND user_id=$user_id";
    			if ( !($db->sql_query($sql)) )
    			{
 ...
 ******************************************************/
                        $sql = "SELECT u.user_id, u.user_email, u.nuke_user_lang, pt.post_text, u.username
 ...
 ******************************************************/
                        ...
                        if ($row = $db->sql_fetchrow($result))
                        {
                                ...
                                if (count($bcc_list_ary))
                                {
                                    ...
                                    {
                                        $name_list = $user_name[$nuke_user_lang] ?? '';
