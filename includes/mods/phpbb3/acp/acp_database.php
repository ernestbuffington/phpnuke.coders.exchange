			...
			case 'backup':
				...
				switch ($action)
				{
					case 'download':
						...
						$extractor->write_start($prefix_phpbb3);
					...
					default:
						include(PHPBB3_INCLUDE_DIR . 'functions_install.' . $phpEx);
			...
			case 'restore':
				...
				switch ($action)
				{
					case 'submit':
						...
						$file_name = PHPBB3_ROOT_DIR . 'store/' . $matches[0];
...
class base_extractor
{
  ...
  $this->download = $download;
		...
		if ($store == true)
		{
			...
			$file = PHPBB3_ROOT_DIR . 'store/' . $filename . $ext;
...
{
	function write_start($prefix_phpbb3)
	{
		$sql_data = "#\n";
		$sql_data .= "# phpBB Backup Script\n";
		$sql_data .= "# Dump of tables for $prefix_phpbb3\n";
