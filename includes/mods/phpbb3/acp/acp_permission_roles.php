  ...
  global $db, $user, $auth, $template, $cache;
		...
		if ($submit || $action == 'remove')
		{
			switch ($action)
			{
				...
				case 'add':
					...
					if ($action == 'edit')
					{
						$sql = 'UPDATE ' . PHPBB3_ACL_ROLES_TABLE . '
					...
					else
					{
						...
						$sql = 'INSERT INTO ' . PHPBB3_ACL_ROLES_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_ary);
						$db->sql_query($sql);
			...
			case 'add':
				...
				if ($options_from)
				{
					$sql = 'SELECT p.auth_option_id, p.auth_setting, o.auth_option
						FROM ' . PHPBB3_ACL_ROLES_DATA_TABLE . ' p, ' . PHPBB3_ACL_OPTIONS_TABLE . ' o
		...
		if ($row = $db->sql_fetchrow($result))
		{
			...
			do
			{
				...
				{
					$db->sql_query('UPDATE ' . PHPBB3_ACL_ROLES_TABLE . " SET role_order = $order WHERE role_id = {$row['role_id']}");
				}
			}
			while ($row = $db->sql_fetchrow($result));
		}
	...
	*/
	function remove_role($role_id, $permission_type)
	{
		...
		$sql = 'DELETE FROM ' . PHPBB3_ACL_ROLES_TABLE . '
			WHERE role_id = ' . $role_id;
		$db->sql_query($sql);
