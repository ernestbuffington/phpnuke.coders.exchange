...
class acp_permissions
{
	...
	function main($id, $mode)
	{
		...
		if (str_starts_with($mode, 'setting_'))
		{
			...
			$hold_ary = $auth_admin->get_mask('set', (sizeof($user_id)) ? $user_id : false, (sizeof($group_id)) ? $group_id : false, (sizeof($forum_id)) ? $forum_id : false, $permission_type, $permission_scope, PHPBB3_PHPBB3_ACL_NO);
		...
		else
		{
			...
			$hold_ary = $auth_admin->get_mask('view', (sizeof($user_id)) ? $user_id : false, (sizeof($group_id)) ? $group_id : false, (sizeof($forum_id)) ? $forum_id : false, $permission_type, $permission_scope, PHPBB3_PHPBB3_ACL_NEVER);
  ...
  global $db, $user;
	...
	*/
	function check_assigned_role($role_id, &$auth_settings)
	{
		...
		$sql = 'SELECT o.auth_option, r.auth_setting
			FROM ' . PHPBB3_ACL_OPTIONS_TABLE . ' o, ' . PHPBB3_ACL_ROLES_DATA_TABLE . ' r
	...
	*/
	function log_action($mode, $action, $permission_type, $ug_type, $ug_id, $forum_id)
	{
		...
		else
		{
			...
			add_log('admin', 'LOG_PHPBB3_ACL_' . strtoupper($action) . '_' . strtoupper($mode) . '_' . strtoupper($permission_type), $l_forum_list, $l_ug_list);
		}
	}
	...
	*/
	function permission_trace($user_id, $forum_id, $permission)
	{
		...
		$auth_setting = (!sizeof($hold_ary)) ? PHPBB3_PHPBB3_ACL_NO : $hold_ary[$user_id][$forum_id][$permission];
	...
	*/
	function retrieve_defined_user_groups($permission_scope, $forum_id, $permission_type)
	{
		...
		$sql = 'SELECT DISTINCT u.user_id, u.username, u.username_clean, u.user_regdate
			FROM ' . PHPBB3_USERS_TABLE . ' u, ' . PHPBB3_ACL_USERS_TABLE . " a
		...
		$sql = 'SELECT DISTINCT g.group_type, g.group_name, g.group_id
			FROM ' . PHPBB3_GROUPS_TABLE . ' g, ' . PHPBB3_ACL_PHPBB3_GROUPS_TABLE . " a
