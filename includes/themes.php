...
-=[Mods]=-	  
      Updated / Fixed HTML                        v5.0.0       04/07/2023
	  Updated / Fixed Theme Groups Allow View     v1.0.0       04/07/2023
	  Now only shows Themes a user is allowed to see based
	  on group membership and not just admin status.
 ************************************************************************/
...
function ThemeGetStatus($theme_name, $active=0) {
    ...
    if(!theme_exists($theme_name)) {
        return "<span style=\"color:red;\"><strong>"._THEMES_THEME_MISSING."</strong></span>";
    }
...
function ThemeGetGroups($groups) {
    ...
    for($i=0, $maxi=count($groups); $i<$maxi; $i++) {
        ...
        $sql = "SELECT group_name FROM " . $prefix . "_bbgroups WHERE nuke_group_id = '" . $groups[$i] . "'";
        $result = $db->sql_query($sql);
        $row = $db->sql_fetchrow($result);
        $db->sql_freeresult($result);
        ...
        case 'user_themes':
			...
			while ($row = $db->sql_fetchrow($result)) 
            {
				...
				if ($perms == 1) {
					...
					{
	                  # Select all groups that need a little hornjizz
                      if(isset($userinfo['groups'])) 
	                  {
	                     foreach ($userinfo['groups'] as $id1 => $name) 
	                     {
                           $in_group[] = $id1;
						   $horndonkle = [];
						   $horndonkle = "SELECT nuke_group_id FROM " . $prefix . "_bbgroups WHERE group_name='$name'";
                           if (!$hornsponkle = $db->sql_query($horndonkle)) {
                               $debugger->handle_error(_THEMES_ERROR_MESSAGE, _THEMES_ERROR);
                           }
						   while ($hornjizz = $db->sql_fetchrow($hornsponkle)) {
						   $this_group = $hornjizz['nuke_group_id'];

	                       if(!empty($name) && $this_group == $groups)
		                   {
                            $themes = add_theme($themes, $theme_name, $custom_name, $groups, $perms, $active);
		                   }
						  }
