...
class sql_db 
{
    ...
    function check_query($query) {
        global $prefix, $cache;
        if (!stristr($query, "REPLACE") && !stristr($query, "UPDATE") && !stristr($query, "INSERT") && !stristr($query, "DELETE")) { return; }
...
//           $query_parts = preg_split('/(union)([\s\ \*\/]+)(all|distinct|select)/i', $query, -1, PREG_SPLIT_NO_EMPTY);
	...
	function sql_query($query = "", $transaction = FALSE)
	{
		...
		//if ($this->query_result) <-- this was giving an undefined query result! 04/06/2023 maybe ghost
		if (isset($this->query_result) ? $this->query_result : false)
		{
		...
		else
		{
			...
			if($this->file == $_SERVER['DOCUMENT_ROOT'].'/admin/modules/themes.php' && $this->line == 801) {
			 $sqlerror['code'] = 'PRE2023'; 
			 $sqlerror['message'] = 'This theme does not have any advanced features, you can add advanced features by adding a theme_info.php file to the root directory of the current theme.';
			 $query = 'theme_info.php NOT FOUND!';
		    }
...
//			return ( $transaction == END_TRANSACTION ) ? true : false;
	...
	*/	
	...
	{
	  if($this->db_connect_id):
	  $result  = '<div class="poweredby"> <a class="poweredby" href="http://www.phpnuke.coders.exchange/" target="_blank">Website Powered by PHP-Nuke v'.NUKE_VERSION.' | &copy; 2000, 2023 Francisco Burzi and The PHP-Nuke Group</a></div>';
